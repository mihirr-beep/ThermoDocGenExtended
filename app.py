...
def _build_request_status_summary(base_query) -> dict:
    ...
    for grouped_row in grouped_status_counts:
        ...
        normalized_status_counts[status_label] = normalized_status_counts.get(status_label, 0) + row_count
    request_rows = base_query.options(joinedload(EMCRequest.tests)).all()
    request_ids = [request_obj.id for request_obj in request_rows]
    planner_entries_by_request_id = {}
    if request_ids:
        planner_entries = PlannerEntry.query.filter(
            PlannerEntry.test_request_id.in_(request_ids)
        ).all()
        for planner_entry in planner_entries:
            request_id = getattr(planner_entry, 'test_request_id', None)
            if not request_id:
                continue
            planner_entries_by_request_id.setdefault(request_id, []).append(planner_entry)
    partially_scheduled_count = 0
    for request_obj in request_rows:
        request_status = str(getattr(request_obj, 'status', '') or '').strip().lower()
        if request_status in {'completed', 'cancelled', 'rejected'}:
            continue
        selected_tests = _get_active_selected_test_labels(request_obj)
        if not selected_tests:
            continue
        assigned_test_keys = set()
        for planner_entry in planner_entries_by_request_id.get(request_obj.id, []):
            planner_status = str(getattr(planner_entry, 'status', '') or '').strip().lower()
            if planner_status == 'cancelled':
                continue
            assigned_key = _normalize_assignment_test_key(
                getattr(planner_entry, 'test_name', None)
            )
            if assigned_key:
                assigned_test_keys.add(assigned_key)
        if not assigned_test_keys:
            continue
        remaining_selected_tests = []
        for selected_test in selected_tests:
            selected_key = _normalize_assignment_test_key(selected_test)
            if selected_key and selected_key in assigned_test_keys:
                continue
            remaining_selected_tests.append(selected_test)
        if remaining_selected_tests:
            partially_scheduled_count += 1
    testing_in_progress_count = sum(
        normalized_status_counts.get(status_label, 0)
        for status_label in (
            'Approved',
            'Assigned',
    ...
    completed_count = normalized_status_counts.get('Completed', 0)
    draft_report_count = normalized_status_counts.get('Draft Report', 0)
    datasheet_uploaded_count = normalized_status_counts.get('Datasheet Uploaded', 0)
    at_review_count = normalized_status_counts.get('At Review', 0)
    rejected_cancelled_count = (
        normalized_status_counts.get('Rejected', 0)
        + normalized_status_counts.get('Cancelled', 0)
    )
    status_cards = [
        {
            'key': 'completed',
        ...
        },
        {
            ...
            'count': testing_in_progress_count,
        },
        {
            'key': 'draft_report',
            'label': 'Draft Report',
            'count': draft_report_count,
        },
        {
            'key': 'datasheet_uploaded',
            'label': 'Datasheet Uploaded',
            'count': datasheet_uploaded_count,
        },
        {
            'key': 'at_review',
            'label': 'At Review',
            'count': at_review_count,
        },
        {
            'key': 'partially_scheduled',
            'label': 'Partially Scheduled',
            'count': partially_scheduled_count,
        },
        {
            'key': 'rejected_cancelled',
            'label': 'Rejected / Cancelled',
            'count': rejected_cancelled_count,
        },
    ]
    exact_status_counts = {
        card['label']: int(card['count'])
        for card in status_cards
    }
    return {
        'total': int(total_count),
        'completed': int(completed_count),
        'testing_in_progress': int(testing_in_progress_count),
        'in_progress': int(testing_in_progress_count),
        'draft_report': int(draft_report_count),
        'datasheet_uploaded': int(datasheet_uploaded_count),
        'at_review': int(at_review_count),
        'pending_approval': int(at_review_count),
        'partially_scheduled': int(partially_scheduled_count),
        'rejected_cancelled': int(rejected_cancelled_count),
        'cancelled': int(rejected_cancelled_count),
...
def _build_planner_conflict_snapshot(record):
    ...
    if isinstance(record, dict):
        ...
        engineer_user_id = record.get(
            'engineer_user_id', record.get('engineer_id'))
        engineer_name = record.get(
            'test_person_name', record.get('engineer_name'))
...
def _format_schedule_conflict_message(test_name, engineer_name, conflicts):
    ...
    if engineer_conflicts:
        ...
        conflicting_test = first_conflict.get(
            'test_name') or 'another schedule'
        conflicting_engineer = first_conflict.get(
            'engineer_name') or engineer_name or 'Selected engineer'
        messages.append(
...
def _get_active_selected_test_labels(test_request: EMCRequest) -> list[str]:
    ...
    for request_test in getattr(test_request, 'tests', []) or []:
        ...
        if str(getattr(request_test, 'workflow_status', '') or '').strip().lower() == 'cancelled':
            continue
        label = EMCRequest._legacy_test_key(
            getattr(request_test, 'test_code', None))
...
def _reconcile_removed_tests_after_admin_edit(
    ...
    for planner_entry in PlannerEntry.query.filter(db.or_(*planner_filters)).all():
        ...
        if planner_key not in removed_match_keys:
            continue
        planner_status = str(
            getattr(planner_entry, 'status', '') or '').strip().lower()
...
def _get_latest_report_planner_entry(test_request):
    ...
    query_filter = (
        db.or_(*planner_filters)
        if len(planner_filters) > 1 else planner_filters[0]
    )
...
def _parse_report_access_feedback_payload(data):
    ...
    if isinstance(raw_ratings, dict):
        ...
        for key, label in rating_fields:
            raw_value = raw_ratings.get(key)
            try:
                numeric_value = int(raw_value)
            except (TypeError, ValueError):
                return None, None, f'{label} rating must be a number between 1 and 10'
            if numeric_value < 1 or numeric_value > 10:
                return None, None, f'{label} rating must be between 1 and 10'
            ratings[key] = numeric_value
        return ratings, comments, None
...
def _parse_comment_timestamp(value) -> datetime:
    ...
    raw = (value or '').strip() if isinstance(
        value, str) else (str(value).strip() if value else '')
    if not raw:
        return datetime.min
    known_formats = [
        '%d %b %Y, %I:%M %p',
...
def _get_combined_review_comment_thread(test_request: EMCRequest) -> list:
    ...
    merged.sort(key=lambda entry: _parse_comment_timestamp(
...
def ensure_planner_table():
    ...
    try:
        ...
        if 'idx_planner_peer_reviewer' not in planner_indexes:
            ...
            db.session.commit()
            logger.info(
                'Added idx_planner_peer_reviewer index to planner_entries')
    except Exception as exc:
        db.session.rollback()
        error_msg = str(exc).lower()
        if 'duplicate' in error_msg or 'already exists' in error_msg:
...
Please do not reply to this email.
    ...
    def send_completion_notification(test_request: EMCRequest, completed_by: str) -> None:
        """Send professional HTML email notification to requester and admin when TCO is completed."""
        requester_email = test_request.requester_email
        if not requester_email or not requester_email.strip():
            logger.warning(
                'No requester email found; skipping completion notification.')
            return
        # Get admin emails
        admin_recipients = [
            u.email for u in User.query.filter_by(
        ...
        try:
            ...
            if len(product_desc) > 300:
                product_desc = product_desc[:300] + '...'
            # Create HTML email body (matching submission email format but with green/completion theme)
            html_body = f"""
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                <div class="header">
                    ...
                    <p style="margin: 10px 0 0 0; opacity: 0.9;">Your EMI/EMC test request has been completed</p>
                </div>

                <div class="content">
                    <p>Dear {test_request.requester_name or 'User'},</p>

                    <p>We are pleased to inform you that your EMI/EMC test request has been completed successfully. All tests have been executed, reports have been reviewed and approved, and your request is now ready for closure.</p>
                    ...
                    <div class="info-box">
                        ...
                        <div class="detail-row">
                            <div class="detail-label">Status:</div>
                            <div class="detail-value"><strong style="color: #10B981;">? Completed</strong></div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Completed By:</div>
                            <div class="detail-value">{completed_by or 'Laboratory Administrator'}</div>
                        </div>
                        <div class="detail-row">
                    ...
                    <p style="margin-top: 25px; padding: 15px; background: #f0fdf4; border-radius: 4px; color: #047857;">
                        <strong>?? Next Steps:</strong><br>
                        � Final reports are available in the system<br>
                        � You may download and review the complete test documentation<br>
                        � Contact us if you have any questions regarding the test results
                    </p>

                    <p>Thank you for choosing our laboratory for your EMI/EMC testing requirements. We appreciate your business and look forward to supporting your future testing needs.</p>
                </div>

                <div class="footer">
                    <p>This is an automated notification from the Test Request Management System.</p>
                    <p>Please do not reply to this email. For inquiries, contact the laboratory administration.</p>
                </div>
            </body>
            </html>
            """
...
We are pleased to inform you that your EMI/EMC test request has been completed successfully.
...
Completed On: {get_ist_now().strftime('%Y-%m-%d %H:%M:%S')} IST

Next Steps:
� Final reports are available in the system
� You may download and review the complete test documentation
� Contact us if you have any questions regarding the test results

Thank you for choosing our laboratory for your EMI/EMC testing requirements.
...
Please do not reply to this email.
            ...
            # Send email to requester
            msg = MIMEMultipart('alternative')
            msg['Subject'] = f'Test Completed: {test_request.product_name or "Unnamed Product"} (TCO: {test_request.tco_id or f"REQ-{test_request.id}"})'
            msg['From'] = formataddr(
                (flask_app.config['SMTP_FROM_NAME'], flask_app.config['SMTP_FROM_EMAIL']))
            msg['To'] = requester_email
            # CC all admin users
            ...
            with smtplib.SMTP(flask_app.config['SMTP_SERVER'], flask_app.config['SMTP_PORT'], timeout=10) as server:
                server.sendmail(
                    flask_app.config['SMTP_FROM_EMAIL'], recipients, msg.as_string())
            logger.info(
                'Test completion notification sent to requester (%s) and %d admin(s)',
                requester_email, len(admin_recipients))
        except Exception as e:
            logger.error('Failed to send completion notification email: %s', e)
            ...
            try:
                ...
                for engineer_email, group_data in engineer_groups.items():
                    ...
                    for assignment in assignments:
                        ...
                        schedule = f"{start_window} → {end_window}"
...
{body_text_plain}
Product / TCO ID: {test_request.product_name or 'Unnamed Product'} / {test_request.tco_id or f'REQ-{test_request.id}'}
Schedule: {start_window} → {end_window}
    ...
    <div class="content">
        ...
        <div class="info-box">
            <div class="detail-row">
                ...
                <div class="detail-value"><strong>{test_request.product_name or 'Unnamed Product'}</strong> · {test_request.tco_id or f'REQ-{test_request.id}'}</div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">{start_window} → {end_window}</div>
            </div>
            <div class="detail-row">
...
"""
    ...
    def add_auth_cache_headers(response):
        ...
        if existing_vary:
            vary_parts = [part.strip()
                          for part in existing_vary.split(',') if part.strip()]
    ...
    def _resolve_test_standard_values(raw_standards, test_code: str, product_standards=None) -> list:
        ...
        if normalized_test_code in ('CE', 'RE'):
            ...
            iec_label = next(
                (label for label, *_ in options if label.startswith('IEC ')), None)
            en_label = next(
                (label for label, *_ in options if label.startswith('EN ')), None)
    ...
    def get_test_requests():
        ...
        try:
            ...
            has_review_messages_expr = db.case(
                (
                    db.and_(
                        ...
                        db.func.trim(EMCRequest.review_comments) != '{}',
                        db.func.lower(db.func.trim(
                            EMCRequest.review_comments)) != 'null',
                    ),
                    True,
                ),
                else_=False,
            ...
            list_query = base_query.with_entities(
                ...
                EMCRequest.assigned_engineer_name.label(
            ...
            if service_type_filter:
                ...
                if candidate_request_ids:
                    ...
                    logger.info(
                        f'Applied service type filter: {service_type_filter}')
            if sort_by:
            ...
            page_request_ids = [
                tr.id for tr in test_requests if tr.id is not None]
            report_entries_by_request_id = {}
            prefetched_service_types_by_request_id = dict(
                request_service_types_by_request_id)
            request_service_types_by_request_id = {}
            try:
                if page_request_ids:
            ...
            for tr in test_requests:
                try:
                    ...
                    result.append({
                        ...
                        'submitted_date':  (
                            (getattr(tr, 'submitted_at', None)
                             or tr.created_at).strftime('%Y-%m-%d %H:%M')
            ...
            #  Statistics counts                                                    #
            # ------------------------------------------------------------------ #
            status_summary = _build_request_status_summary(base_query)
            total_count = int(status_summary.get('total', 0))
            completed_count = int(status_summary.get('completed', 0))
            testing_in_progress_count = int(status_summary.get('testing_in_progress', 0))
            draft_report_count = int(status_summary.get('draft_report', 0))
            datasheet_uploaded_count = int(status_summary.get('datasheet_uploaded', 0))
            at_review_count = int(status_summary.get('at_review', 0))
            partially_scheduled_count = int(status_summary.get('partially_scheduled', 0))
            ...
            logger.info(
                ...
                total_count,
                completed_count,
                testing_in_progress_count,
                draft_report_count,
                datasheet_uploaded_count,
                at_review_count,
                partially_scheduled_count,
                rejected_cancelled_count,
            )
            # ------------------------------------------------------------------ #
            #  Pagination metadata                                                  #
            ...
            response_data = {
                ...
                'counts': {
                    'total':        total_count,
                    'in_progress':  testing_in_progress_count,
                    'completed':    completed_count,
                    'cancelled':    rejected_cancelled_count,
                    'draft_report': draft_report_count,
                    'datasheet_uploaded': datasheet_uploaded_count,
                    'at_review': at_review_count,
                    'partially_scheduled': partially_scheduled_count,
                    'rejected_cancelled': rejected_cancelled_count,
                    'exact_status_counts': exact_status_counts,
                    'status_cards': status_cards,
                },
                'pagination': {
        ...
        try:
            if raw_code == 'CE':
                ...
                parent_request = db.session.get(
                    EMCRequest, entry.test_request_id)
                ...
                safe_tco_id = secure_filename(
                    str(entry.tco_id or parent_request.tco_id or 'TCO'))
                safe_test_name = secure_filename(
                    str(entry.test_name or 'Surge'))
    ...
    def _apply_peer_review_action(
        ...
        if normalized_action == 'approve':
            if not entry.datasheet_file_path:
                generation_success, generation_message, _ = _generate_final_datasheet_after_peer_review(
                    entry)
    ...
    def get_peer_review_entries():
        ...
        try:
            ...
            # Separate active and completed/cancelled parent requests
            active_results = [r for r in all_results if (r.parent_status or '').lower(
            ).strip() not in ('completed', 'cancelled', 'rejected')]
            completed_results = [r for r in all_results if (
                r.parent_status or '').lower().strip() in ('completed', 'cancelled', 'rejected')]
            results = active_results + completed_results
            logger.info(
                f'Found {len(results)} peer review entries (active: {len(active_results)}, completed: {len(completed_results)})')
            ...
            for entry, actual_tco_id, job_number, product_name, requester_name, actual_request_id, parent_status in results:
                ...
                if entry.peer_reviewer_user_id:
                    peer_reviewer_user = db.session.get(
                        User, entry.peer_reviewer_user_id)
                ...
                try:
                    from datasheet_gen import records as datasheet_records
                    datasheet_record = datasheet_records.get_record_for_assignment(
                        entry.id)
                    if datasheet_record:
    ...
    def get_planner_entry(planner_id):
        ...
        try:
            ...
            uploaded_by_name = None
            if entry.datasheet_uploaded_by:
                uploaded_by_user = db.session.get(
                    User, entry.datasheet_uploaded_by)
                if uploaded_by_user:
                    uploaded_by_name = uploaded_by_user.username
            peer_reviewer_name = None
            if entry.peer_reviewer_user_id:
        ...
        try:
            ...
            request_test = next((
                ...
                and _normalize_assignment_test_key(
                    EMCRequest._legacy_test_key(
                        getattr(row, 'test_code', None))
                ) == requested_key
            ), None)
            if not request_test:
                return jsonify({
            ...
            if test_request.tco_id:
                planner_filters.append(
                    PlannerEntry.tco_id == test_request.tco_id)
            ...
            db.session.rollback()
            logger.error(
                'Error cancelling unassigned test for request %s: %s', request_id, exc)
            return jsonify({
                'success': False,
    ...
    def update_test_request_job_number(request_id):
        ...
        try:
            ...
            db.session.rollback()
            logger.error(
                'Error updating Job Number for request %s: %s', request_id, e)
            return jsonify({
                'success': False,
    ...
    def _extract_custom_range_text(spec_obj) -> str:
        ...
        from_unit = _coerce_export_text(
            normalized.get('fromUnit') if isinstance(
                normalized, dict) else normalized.get('from_unit')
        )
        to_value = _coerce_export_text(
            normalized.get('to') if isinstance(normalized, dict) else ''
        )
        to_unit = _coerce_export_text(
            normalized.get('toUnit') if isinstance(
                normalized, dict) else normalized.get('to_unit')
        )
        ...
        for key in keys:
            ...
            if parsed_spec:
                for selector_key in ('spec', 'frequency', 'frequencySpec', 'frequency_spec'):
                    selector_value = _coerce_export_text(
                        parsed_spec.get(selector_key))
                    if not selector_value:
                        continue
                    if 'custom' in selector_value.lower():
                        custom_range_text = _extract_custom_range_text(
                            parsed_spec)
    ...
    def _get_software_defaults_for_test_keyword(test_keyword: str) -> list[dict]:
        ...
        try:
            software_rows = Equipment.query.filter(
                ...
                db.func.lower(Equipment.type).in_(
    ...
    def _build_request_payload(test_request, include_review_thread: bool = True) -> dict:
        ...
        request_data['planner_entries'] = [entry.to_dict()
        ...
        if include_review_thread:
            ...
            if review_thread:
                ...
                request_data['reviewed_by'] = latest_comment.get(
                    'username') or request_data.get('reviewed_by')
                request_data['reviewed_at'] = latest_comment.get(
                    'created_at') or request_data.get('reviewed_at')
        ...
        for field_name in ('sample_condition', 'capability_available', 'test_duration', 'lab_manager_name', 'lab_manager_signature'):
            if not request_data.get(field_name):
                request_data[field_name] = getattr(
                    test_request, field_name, None) or ''
        for field_name in ('sample_received_date', 'test_commencement_date', 'test_completion_date', 'lab_manager_date'):
            if not request_data.get(field_name):
                request_data[field_name] = _iso_date_text(
                    getattr(test_request, field_name, None))
        if not request_data.get('sample_received_date'):
            request_received = getattr(test_request, 'submitted_at', None) or getattr(
                test_request, 'created_at', None)
            request_data['sample_received_date'] = _iso_date_text(
                request_received)
        ...
        if not request_data.get('test_commencement_date') and earliest_start_date:
            request_data['test_commencement_date'] = _iso_date_text(
                earliest_start_date)
        if not request_data.get('test_completion_date') and latest_end_date:
            request_data['test_completion_date'] = _iso_date_text(
                latest_end_date)
        if not request_data.get('test_duration') and total_planner_hours > 0:
            request_data['test_duration'] = _format_duration_text(
                total_planner_hours)
        if not request_data.get('lab_manager_date') and getattr(test_request, 'lab_manager_signed_at', None):
            request_data['lab_manager_date'] = _iso_date_text(
                test_request.lab_manager_signed_at)
        request_data['selected_tests'] = _get_active_selected_test_labels(
            test_request)
    ...
    def _set_table_cell_text(doc, table_idx: int, row_idx: int, col_idx: int, value, italic: bool = False, bold: bool = False) -> None:
        ...
        try:
            ...
            cell.text = text_value if text_value else _get_export_empty_placeholder(
                doc)
            if (italic or bold) and text_value:
    ...
    def _set_table_cell_labeled_lines(doc, table_idx: int, row_idx: int, col_idx: int, items) -> None:
        ...
        try:
            ...
            filtered_items = [
                (
                    _coerce_export_text(label).strip(),
                    _coerce_export_text(value).strip(),
                )
                for label, value in (items or [])
                if _coerce_export_text(label).strip() or _coerce_export_text(value).strip()
            ]
            if not filtered_items:
                cell.text = _get_export_empty_placeholder(doc)
                return
            paragraph = cell.paragraphs[0] if cell.paragraphs else cell.add_paragraph()
            for idx, (label_text, value_text) in enumerate(filtered_items):
                if idx > 0:
                ...
                if label_text:
                    ...
                    label_run.bold = True
                if value_text:
                    paragraph.add_run(f' {value_text}' if label_text else value_text)
        except Exception as exc:
            logger.debug(
                'Skipping labeled table write (table=%s row=%s col=%s): %s',
                table_idx,
                row_idx,
                col_idx,
                exc
    ...
    def _remove_table_rows(doc, table_idx: int, row_indexes) -> None:
        """Remove table rows by index, processing from bottom to top."""
        if row_indexes is None:
            return
        try:
            table = doc.tables[table_idx]
            normalized_indexes = sorted({
                int(row_index)
                for row_index in row_indexes
                if isinstance(row_index, int) or str(row_index).isdigit()
            }, reverse=True)
            for row_index in normalized_indexes:
                if 0 <= row_index < len(table.rows):
                    table._tbl.remove(table.rows[row_index]._tr)
        except Exception as exc:
            logger.debug(
                'Unable to remove rows %s from table %s: %s',
                row_indexes,
                table_idx,
                exc
            )
    ...
    def _insert_paragraph_after_table(table, text: str = ''):
        """Insert and return a new paragraph immediately after the given table."""
        new_paragraph_xml = OxmlElement('w:p')
        table._tbl.addnext(new_paragraph_xml)
        new_paragraph = Paragraph(new_paragraph_xml, table._parent)
        if text:
            new_paragraph.add_run(text)
        return new_paragraph
    def _set_following_paragraph_text(doc, label_snippet: str, value) -> bool:
        ...
        for idx, paragraph in enumerate(doc.paragraphs):
            ...
            if normalized_label in paragraph_text and idx + 1 < len(doc.paragraphs):
                text_value = _coerce_export_text(value)
                doc.paragraphs[idx +
                               1].text = text_value if text_value else _get_export_empty_placeholder(doc)
                return True
        return False
    ...
    def _insert_block_diagram_in_template(doc, block_diagram_value) -> None:
        ...
        for table in doc.tables:
            for row_idx, row in enumerate(table.rows):
                for cell in row.cells:
                    cell_text_upper = ' '.join(
                        (cell.text or '').split()).upper()
                    if 'BLOCK DIAGRAM OF THE TEST SETUP' not in cell_text_upper:
                        continue
                    if cell.paragraphs:
                        anchor_paragraph = cell.paragraphs[0]
                    if row_idx + 1 < len(table.rows) and table.rows[row_idx + 1].cells:
                        target_cell = table.rows[row_idx + 1].cells[0]
                    break
                if anchor_paragraph:
                    break
            if anchor_paragraph:
        ...
        if not anchor_paragraph:
            for paragraph in doc.paragraphs:
                text_upper = ' '.join((paragraph.text or '').split()).upper()
        ...
        try:
            if target_cell is not None:
                target_cell.text = ''
                image_paragraph = target_cell.paragraphs[0] if target_cell.paragraphs else target_cell.add_paragraph(
                )
            else:
                image_paragraph = _insert_paragraph_after(anchor_paragraph, '')
            image_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
            image_run = image_paragraph.add_run()
            image_run.add_picture(BytesIO(image_bytes), width=Inches(3.4))
        except Exception as exc:
            logger.warning(
                'Unable to insert block diagram image into TRF template: %s', exc)
    def _set_table_cell_signature(doc, table_idx: int, row_idx: int, col_idx: int, value, width_inches: float = 1.8) -> None:
        """Render a signature into a table cell as an image when possible, else as text."""
        image_bytes = _load_block_diagram_image_bytes(value)
        if image_bytes:
            try:
                table = doc.tables[table_idx]
                if row_idx >= len(table.rows):
                ...
                paragraph.add_run().add_picture(BytesIO(image_bytes), width=Inches(width_inches))
                return
            except Exception as exc:
                logger.warning(
                    'Unable to insert signature image into TRF template cell t=%s r=%s c=%s: %s',
                    table_idx,
                    row_idx,
                    col_idx,
                    exc,
        ...
        _set_table_cell_text(
            ...
            row_idx,
            col_idx,
            _format_signature_export_value(value),
            italic=True,
        )
        try:
            table = doc.tables[table_idx]
            ...
            logger.debug(
                'Unable to apply signature text styling for table=%s row=%s col=%s: %s',
                table_idx,
                row_idx,
                col_idx,
                exc,
    ...
    def _format_effective_date_for_template_header(value: str) -> str:
        ...
        try:
            ...
            try:
                ...
                return str(value)
    def _format_test_duration_days_only(value, start_date=None, end_date=None) -> str:
        """Return duration as a day-count derived from commencement/completion dates."""
        def _parse_duration_date(raw_value):
            ...
            try:
                ...
                return None
        parsed_start_date = _parse_duration_date(start_date)
        parsed_end_date = _parse_duration_date(end_date)
        if parsed_start_date and parsed_end_date and parsed_end_date >= parsed_start_date:
            return str((parsed_end_date - parsed_start_date).days)
    ...
    def _set_template_effective_date_in_header(doc, effective_date_text: str) -> None:
        """Fill the header effective date only when the template leaves it blank."""
            ...
            for section in doc.sections:
                ...
                for table in header.tables:
                    for row in table.rows:
                        ...
                        if "effective date" not in label:
                            continue
                        current_value = (row.cells[2].text or "").strip()
                        if current_value and current_value not in {"--------", "N/A"}:
                            return
                        row.cells[2].text = effective_date_text
                        return
        except Exception as exc:
            logger.debug(
                'Unable to set template header effective date: %s', exc)
    ...
    def _build_test_request_word_export_from_template(request_data: dict):
        ...
        try:
            ...
            if len(doc.tables) > 0 and len(doc.tables[0].rows) > 0:
                ...
                for idx, is_selected in enumerate(service_checks):
                    if idx < len(row0.cells):
                        ...
                        row0.cells[idx].text = f"{'☑' if is_selected else '��?'} {current_label}"
            ...
            _set_table_cell_text(
                doc, 1, 0, 2, request_data.get('product_name'))
            _set_table_cell_text(
                doc, 1, 1, 2, request_data.get('manufacturer'))
            _set_table_cell_text(
                doc, 1, 2, 2, request_data.get('model_number'))
            _set_table_cell_text(doc, 1, 3, 2, serial_number_display)
            # Table 2: product details
            _set_table_cell_text(
                doc, 2, 0, 2, request_data.get('test_samples'))
            ...
            _set_table_cell_text(
                doc, 2, 3, 2, request_data.get('operating_frequency'))
            category_selected = _selected_option_set(
                request_data.get('category'))
            category_checkbox_text = (
                f"{'☑' if 'medical' in category_selected else '��?'} Medical    "
                f"{'☑' if 'laboratory' in category_selected else '��?'} Laboratory    "
                f"{'☑' if 'custom' in category_selected else '��?'} Custom"
            )
            _set_table_cell_text(doc, 2, 4, 2, category_checkbox_text)
            product_type_raw = _coerce_export_text(
                request_data.get('type')).lower()
            ...
            floor_text = f"{'☑' if is_floor_standing else '��?'} Floor Standing"
            tabletop_text = f"{'☑' if is_tabletop else '��?'} Tabletop"
            others_text = f"{'☑' if is_others else '��?'} others: specify"
            ...
            _set_table_cell_text(
                doc, 7, 1, 2, request_data.get('product_description'))
            _set_table_cell_text(
                doc, 7, 3, 2, request_data.get('test_configuration'))
            _set_table_cell_text(
                doc, 7, 4, 2, request_data.get('operation_modes'))
            _set_table_cell_text(
                doc, 7, 5, 2, request_data.get('monitoring_parameters'))
            _set_table_cell_text(
                doc, 7, 6, 2, request_data.get('additional_info'))
            ...
            test_hours_mapping = [
                ...
                (11, ('VoltageDips', 'VOLTAGE',
                 'Voltage Dips, Short Interruptions Immunity')),
            ]
            for row_idx, keys in test_hours_mapping:
                _set_table_cell_text(doc, 8, row_idx, 1,
                                     _find_test_hour(*keys))
            # Table 9: standards/environment/group/class
            standards_selected = _selected_option_set(
                request_data.get('product_standards')
            )
            standards_text = (
                f"{'☑' if _selection_contains(standards_selected, 'iec 61326-1') else '��?'} IEC 61326-1    "
                f"{'☑' if _selection_contains(standards_selected, 'en 61326-1') else '��?'} EN 61326-1    "
                f"{'☑' if _selection_contains(standards_selected, 'iec 60601-1-2') else '��?'} IEC 60601-1-2\n"
                f"{'☑' if _selection_contains(standards_selected, 'en 60601-1-2') else '��?'} EN 60601-1-2    "
                f"{'☑' if _selection_contains(standards_selected, 'ansi c63.4', 'ansi c63-4') else '��?'} ANSI C63.4    "
                f"{'☑' if _selection_contains(standards_selected, 'other') else '��?'} Other: please specify"
            )
            ...
            environment_text = (
                f"{'☑' if _selection_contains(environment_selected, 'basic') else '��?'} Basic Electromagnetic    "
                f"{'☑' if _selection_contains(environment_selected, 'industrial') else '��?'} Industrial    "
                f"{'☑' if _selection_contains(environment_selected, 'controlled') else '��?'} Controlled    "
                f"{'☑' if _selection_contains(environment_selected, 'na', 'n/a') else '��?'} NA"
            )
            _set_table_cell_text(doc, 9, 1, 1, environment_text)
            group_selected = _selected_option_set(request_data.get('group'))
            _set_table_cell_text(
                doc, 9, 2, 1,
                f"{'☑' if _selection_contains(group_selected, 'group 1') else '��?'} Group 1"
            )
            _set_table_cell_text(
                doc, 9, 2, 2,
                f"{'☑' if _selection_contains(group_selected, 'group 2') else '��?'} Group 2"
            )
            class_selected = _selected_option_set(
                request_data.get('class_type'))
            _set_table_cell_text(
                doc, 9, 3, 1,
                f"{'☑' if _selection_contains(class_selected, 'class a') else '��?'} Class A"
            )
            _set_table_cell_text(
                doc, 9, 3, 2,
                f"{'☑' if _selection_contains(class_selected, 'class b') else '��?'} Class B"
            )
            ...
            _accumulate_selected_tests(request_data.get(
                'selected_tests_for_development'))
            ...
            test_block_rows = [
                ...
                (17, 'Electrostatic Discharge Immunity',
                 ('ESD', 'electrostatic discharge')),
                (21, 'Radiated Susceptibility/Immunity',
                ...
                (31, 'Electrical fast transient/burst Immunity',
                 ('EFT', 'fast transient')),
            ...
            def _set_table10_block_checkbox(label: str, aliases: tuple, fallback_row_idx: int) -> None:
                checked_symbol = '\u2611' if _is_test_selected(
                    *aliases) else '\u2610'
                ...
                try:
                    ...
                    logger.debug(
                        'Unable to set table10 block checkbox by label (%s): %s', label, exc)
                # Fallback to fixed row index.
                if not updated:
                    _set_table_cell_text(
                        doc, 10, fallback_row_idx, 0, target_text)
            ...
            test_name_rows = [
                ...
                (4, 'Voltage Changes, Voltage Fluctuations and Flicker Emission',
                 ('VoltageFlicker', 'Flicker', 'voltage changes')),
                (5, 'Electrostatic Discharge Immunity (ESD)',
                 ('ESD', 'electrostatic discharge')),
                (6, 'Radiated Susceptibility/Immunity (RS/RI)',
                 ('RS_RI', 'RS', 'radiated susceptibility')),
                (7, 'Electrical fast transient/burst Immunity (EFT)',
                 ('EFT', 'fast transient')),
                ...
                (11, 'Voltage Dips, Short Interruptions Immunity',
                 ('VoltageDips', 'VOLTAGE')),
            ]
            for row_idx, label, aliases in test_name_rows:
                checked_symbol = '☑' if _is_test_selected(
                    *aliases) else '��?'
                _set_table_cell_text(doc, 8, row_idx, 0,
                                     f'{checked_symbol} {label}')
            def _set_test_row_if_selected(row_idx: int, value, *aliases: str) -> None:
                if not _is_test_selected(*aliases):
                    return
                text_value = _coerce_export_text(value)
                if text_value:
                    _set_table_cell_text(doc, 12, row_idx, 2, text_value)
            ...
            _set_test_row_if_selected(1, request_data.get(
                'ce_voltage_freq'), 'ce', 'conducted emission')
            _set_test_row_if_selected(
                2,
                _extract_frequency_range_display(
            ...
            _set_test_row_if_selected(3, f"Power: {_coerce_export_text(request_data.get('ce_cables'))}",
                                      'ce', 'conducted emission')
            _set_test_row_if_selected(4, request_data.get(
                'ce_class'), 'ce', 'conducted emission')
            # RE
            ...
            _set_test_row_if_selected(7, request_data.get(
                're_voltage_freq'), 're', 'radiated emission')
            _set_test_row_if_selected(
                8,
                _extract_frequency_range_display(
            ...
            _set_test_row_if_selected(9, request_data.get(
                're_class'), 're', 'radiated emission')
            # Harmonic
            ...
            harmonic_voltage = _coerce_export_text(
                request_data.get('harmonic_voltage_freq'))
            harmonic_class = _coerce_export_text(
                request_data.get('harmonic_class'))
            ...
            _set_test_row_if_selected(
                12, harmonic_detail, 'harmonic', 'harmonic current emission')
            ...
            _set_test_row_if_selected(18, request_data.get(
                'esd_voltage_freq'), 'esd', 'electrostatic discharge')
            ...
            _set_test_row_if_selected(
                19, esd_level, 'esd', 'electrostatic discharge')
            # RS/RI
            _set_standard_row_if_selected(
                21,
            ...
            _set_test_row_if_selected(23,
                                      ...
                                      'rs_ri', 'rs', 'radiated susceptibility')
            rs_field = _coerce_export_text(
                request_data.get('rs_field_strength1'))
            rs_field2 = _coerce_export_text(
                request_data.get('rs_field_strength2'))
            rs_field3 = _coerce_export_text(
                request_data.get('rs_field_strength3'))
            rs_field_text = ' / '.join([x for x in [rs_field,
                                       rs_field2, rs_field3] if x])
            _set_test_row_if_selected(
                24, rs_field_text, 'rs_ri', 'rs', 'radiated susceptibility')
            # RS/RI Interim
            _set_standard_row_if_selected(
                26,
            ...
            _set_test_row_if_selected(28,
                                      ...
                                      'rs_ri_interim', 'rs_interim', 'interim method')
            rs_i_field = _coerce_export_text(
                request_data.get('rs_interim_field_strength1'))
            rs_i_field2 = _coerce_export_text(
                request_data.get('rs_interim_field_strength2'))
            rs_i_field3 = _coerce_export_text(
                request_data.get('rs_interim_field_strength3'))
            rs_i_text = ' / '.join([x for x in [rs_i_field,
                                   rs_i_field2, rs_i_field3] if x])
            _set_test_row_if_selected(
                29, rs_i_text, 'rs_ri_interim', 'rs_interim', 'interim method')
            # EFT
            _set_standard_row_if_selected(
                31,
            ...
            _set_test_row_if_selected(32, request_data.get(
                'eft_voltage_freq'), 'eft', 'fast transient')
            eft_cables = []
            eft_power = _coerce_export_text(
                request_data.get('eft_cables_power'))
            eft_signal = _coerce_export_text(
                request_data.get('eft_cables_signal'))
            if eft_power:
                eft_cables.append(f"Power: {eft_power}")
            if eft_signal:
                eft_cables.append(f"Signal: {eft_signal}")
            _set_test_row_if_selected(33, ' | '.join(
                eft_cables), 'eft', 'fast transient')
            eft_level1 = _coerce_export_text(
                request_data.get('eft_test_level1'))
            eft_level2 = _coerce_export_text(
                request_data.get('eft_test_level2'))
            ...
            _set_test_row_if_selected(
                37, request_data.get('surge_voltage_freq'), 'surge')
            surge_cables = []
            surge_power = _coerce_export_text(
                request_data.get('surge_cables_power'))
            surge_signal = _coerce_export_text(
                request_data.get('surge_cables_signal'))
            if surge_power:
            ...
            _set_test_row_if_selected(
                39, ' / '.join([x for x in [surge_cm1, surge_cm2] if x]), 'surge')
            _set_test_row_if_selected(
                40, ' / '.join([x for x in [surge_dm1, surge_dm2] if x]), 'surge')
            # CRF
            _set_standard_row_if_selected(
                42,
            ...
            _set_test_row_if_selected(43, request_data.get(
                'crf_voltage_freq'), 'crf', 'conducted rf')
            _set_test_row_if_selected(
                44,
                _extract_frequency_range_display(
            ...
            crf_cables = []
            crf_power = _coerce_export_text(
                request_data.get('crf_cables_power'))
            crf_signal = _coerce_export_text(
                request_data.get('crf_cables_signal'))
            if crf_power:
                crf_cables.append(f"Power: {crf_power}")
            if crf_signal:
                crf_cables.append(f"Signal: {crf_signal}")
            _set_test_row_if_selected(45, ' | '.join(
                crf_cables), 'crf', 'conducted rf')
            crf_level1 = _coerce_export_text(
                request_data.get('crf_test_level1'))
            crf_level2 = _coerce_export_text(
                request_data.get('crf_test_level2'))
            ...
            _set_test_row_if_selected(
                53, vd_range, 'voltagedips', 'voltage', 'voltage dips')
            _set_test_row_if_selected(54, request_data.get('voltage_dips_voltage_freq'),
                                      'voltagedips', 'voltage', 'voltage dips')
            vd_levels = ' / '.join([
                x for x in [
                    _coerce_export_text(request_data.get(
            ...
            _set_test_row_if_selected(
                55, vd_levels, 'voltagedips', 'voltage', 'voltage dips')
            vd_times = ' / '.join([
                x for x in [
                    _coerce_export_text(
                        request_data.get('voltage_dips_time1')),
                    _coerce_export_text(
                        request_data.get('voltage_dips_time2')),
                    _coerce_export_text(
                        request_data.get('voltage_dips_time3')),
                    _coerce_export_text(request_data.get('voltage_dips_time4'))
                ] if x
            ])
            _set_test_row_if_selected(
                57, vd_times, 'voltagedips', 'voltage', 'voltage dips')
            ...
            requester_div = _coerce_export_text(
                request_data.get('requester_division'))
            _set_table_cell_text(
                doc, 12, 0, 3,
                f"{requester_dept}{f' / {requester_div}' if requester_div else ''}"
            )
            _set_table_cell_text(
                doc, 14, 1, 1, request_data.get('requester_email'))
            _set_table_cell_text(
                doc, 14, 1, 3, request_data.get('requester_contact'))
            _set_table_cell_text(
                doc, 12, 2, 1, request_data.get('requester_designation'))
            _set_table_cell_text(
                doc, 12, 2, 3, request_data.get('requester_date'))
            ...
            _set_table_cell_text(doc, 15, 0, 1, request_data.get(
                'job_number') or request_data.get('job_id'))
            _set_table_cell_text(
                doc, 13, 1, 1, request_data.get('sample_condition'))
            _set_table_cell_text(
                doc, 13, 2, 1, request_data.get('capability_available'))
            _set_table_cell_text(
                doc, 13, 3, 1, request_data.get('sample_received_date'))
            _set_table_cell_text(
                doc,
                13,
                5,
                2,
                _format_test_duration_days_only(
                    request_data.get('test_duration'),
                    request_data.get('test_commencement_date'),
                    request_data.get('test_completion_date'))
            )
            _set_table_cell_text(
                doc, 13, 6, 2, request_data.get('test_commencement_date'))
            _set_table_cell_text(
                doc, 13, 7, 2, request_data.get('test_completion_date'))
            # Table 14: lab manager sign-off
            _set_table_cell_text(
                doc, 16, 0, 1, request_data.get('lab_manager_name'))
            _set_table_cell_text(
                doc, 16, 0, 3, request_data.get('test_commencement_date') or request_data.get('lab_manager_date'))
            _set_table_cell_signature(
                doc, 16, 1, 1, request_data.get('lab_manager_signature'))
    ...
    def _build_generic_test_request_word_export(request_data: dict):
        ...
        doc = DocxDocument()
        request_title = request_data.get(
            'tco_id') or f"Request {request_data.get('id', 'N/A')}"
        effective_date = _resolve_request_effective_date(request_data)
        if effective_date:
        ...
        doc.add_paragraph(
            f"Generated On: {get_ist_now().strftime('%Y-%m-%d %H:%M:%S')} IST")
        sections = [
            (
                'Basic Information',
                [
            ...
            (
                ...
                [
                    ...
                    ('Manufacturer Address', request_data.get(
                    ...
                    ('Samples Available In Lab', request_data.get(
                        'samples_available_in_lab')),
                    ('Has Model Variance', request_data.get('has_model_variance')),
                    ('Model Variance', request_data.get('model_variance')),
                    ('Project Details/Intent',
                     request_data.get('project_details_intent')),
                ]
            ),
            (
                'Product Details',
                [
                    ...
                    ('Has Wireless Interface', request_data.get(
            ...
            (
                ...
                [
                    ...
                    ('Monitoring Parameters', request_data.get(
            ...
            (
                ...
                [
                    ...
                    ('Product Environment Other', request_data.get(
                    ...
                    ('Selected Tests (Development)', request_data.get(
                        'selected_tests_for_development')),
            ...
            (
                ...
                [
                    ('Assigned Engineer Name', request_data.get(
                        'assigned_engineer_name')),
                    ...
                    ('Capability Available', request_data.get(
                        'capability_available')),
                    ('Sample Received Date', request_data.get(
                        'sample_received_date')),
                    ('Test Duration', request_data.get('test_duration')),
                    ('Test Commencement Date', request_data.get(
                        'test_commencement_date')),
                    ('Test Completion Date', request_data.get(
            ...
            (
                ...
                [
                    ...
                    ('Requester Department', request_data.get(
                    ...
                    ('Requester Designation', request_data.get(
                        'requester_designation')),
                    ('Requester Date', request_data.get('requester_date')),
                    ('Expected Completion Date', request_data.get(
                        'requester_expected_completion_date')),
                    ('Lab Manager Name', request_data.get('lab_manager_name')),
                    ('Lab Manager Date', request_data.get('lab_manager_date')),
                    ('Lab Manager Signed At', request_data.get(
                        'lab_manager_signed_at')),
                ]
            ),
            (
                'Review and Sign-Off',
            ...
            (
                'Attachments',
                [
                    ('Model Variance Document', request_data.get(
                        'model_variance_document')),
                    ('Block Diagram', request_data.get('block_diagram')),
                    ('Requester Signature', request_data.get('requester_signature')),
                    ('Lab Manager Signature', request_data.get(
    ...
    def _get_active_trf_template_path() -> str:
        """Resolve the active TRF template path."""
        candidate_paths = [
            os.path.join(flask_app.root_path, 'word_templates',
                         'IEC-FRM-502 EMI EMC Test Request form.docx'),
            r'c:\Users\saimounik.chandavolu\Downloads\IEC-FRM-502 EMI EMC Test Request form.docx',
            flask_app.config.get('TRF_TEMPLATE_PATH'),
            os.environ.get('TRF_TEMPLATE_PATH'),
            os.path.join(flask_app.root_path, 'word_templates',
                         'EMI EMC TRF_Draft - Rev1.docx'),
    ...
    def _build_test_request_word_export_rev1(request_data: dict):
        ...
        try:
            ...
            service_types = [str(item).strip().lower() for item in _coerce_export_list(
                request_data.get('service_types'))]
            service_flags = [
                any('development' in item for item in service_types),
                any('pre' in item and 'compliance' in item for item in service_types),
                any('compliance' in item and 'pre' not in item for item in service_types),
            ...
            for idx, label in enumerate(['Developmental Assistance (DA)', 'Pre-Compliance', 'Compliance']):
                _set_table_cell_text(
                    doc, 0, 0, idx, _box(label, service_flags[idx]))
            additional_models = [
                _coerce_export_text(item) for item in _coerce_export_list(
                    request_data.get('additional_models')
                ) if _coerce_export_text(item)
            ]
            extra_serials = [
                _coerce_export_text(item) for item in _coerce_export_list(
                    request_data.get('serial_numbers')
                ) if _coerce_export_text(item)
            ]
            primary_serial = _coerce_export_text(
                request_data.get('serial_number'))
            serials = primary_serial or ', '.join(extra_serials)
            def _norm_table_label(value: str) -> str:
                return re.sub(r'\s+', ' ', str(value or '').strip()).lower()
            def _set_table1_value_by_label(label: str, value, col_idx: int = 1) -> bool:
                try:
                    table = doc.tables[1]
                except Exception:
                    return False
                label_norm = _norm_table_label(label)
                for row_idx, row in enumerate(table.rows):
                    if not row.cells:
                        continue
                    cell_label = _norm_table_label(row.cells[0].text)
                    if cell_label == label_norm or label_norm in cell_label:
                        _set_table_cell_text(doc, 1, row_idx, col_idx, value)
                        return True
                return False
            identity_fields = [
                ('Name of the Product', request_data.get('product_name')),
                ('Manufacturer Name', request_data.get('manufacturer')),
                ('Manufacturer Address', request_data.get('manufacturer_address')),
                ('Model Number / SKU Number', request_data.get('model_number')),
                ('No. Test Samples', request_data.get('test_samples')),
                ('Serial Number (s)', serials),
                ('Additional Model Number / SKU Number for Sample 2',
                 additional_models[0] if additional_models else ''),
                ('Serial Number for Sample 2',
                 extra_serials[0] if extra_serials else ''),
                ('Group', request_data.get('requester_group')),
                ('Division (BU)', request_data.get('requester_division')),
                ('Site', request_data.get('requester_site')),
                ('Model Variants', request_data.get('model_variance')),
                ('Project details/Intent', request_data.get('project_details_intent')),
            ]
            for label, value in identity_fields:
                _set_table1_value_by_label(label, value)
            samples_lab = _norm_yes_no(
                request_data.get('samples_available_in_lab'))
            if not _set_table1_value_by_label(
                'Are samples available in Lab?',
                f"{_box('Yes', samples_lab == 'yes')}    {_box('No', samples_lab == 'no')}"
            ):
                _set_table_cell_text(
                    doc, 1, 6, 1, f"{_box('Yes', samples_lab == 'yes')}    {_box('No', samples_lab == 'no')}")
            model_variance = _norm_yes_no(
                request_data.get('has_model_variance'))
            if not _set_table1_value_by_label(
                'Model Variance?',
                f"{_box('Yes', model_variance == 'yes')}    {_box('No', model_variance == 'no')}"
            ):
                _set_table_cell_text(
                    doc, 1, 11, 1, f"{_box('Yes', model_variance == 'yes')}    {_box('No', model_variance == 'no')}")
            ...
            _set_table_cell_text(
                doc, 2, 3, 1, request_data.get('operating_frequency'))
            ...
            type_other = _coerce_export_text(request_data.get('type_others'))
            _set_table_cell_text(doc, 2, 5, 1, _box(
                'Floor Standing', 'floor' in type_raw))
            _set_table_cell_text(doc, 2, 5, 3, _box(
                'Tabletop', 'table' in type_raw))
            _set_table_cell_text(doc, 2, 5, 6, _box(f"others: specify {type_other}".strip(
            ), 'other' in type_raw and 'floor' not in type_raw and 'table' not in type_raw))
            accessories = [
                item for item in _coerce_export_list(request_data.get('accessories'))
                if isinstance(item, dict) and any([
                    _pick_item_value(item, 'equipmentName',
                                     'equipment_name', 'name'),
                    _pick_item_value(item, 'make', 'manufacturer'),
                    _pick_item_value(item, 'modelNo', 'model_no', 'model'),
                    _pick_item_value(item, 'serialNo',
                                     'serial_no', 'serialNumber'),
                    _pick_item_value(item, 'functionPurpose',
                                     'function_purpose', 'purpose', 'function'),
                ])
            ]
            _ensure_table_repeated_rows(doc, 3, 2, len(accessories))
            for idx, accessory in enumerate(accessories, start=2):
                row_idx = idx
                _set_table_cell_text(doc, 3, row_idx, 0, _pick_item_value(
                    accessory, 'equipmentName', 'equipment_name', 'name'))
                _set_table_cell_text(doc, 3, row_idx, 1, _pick_item_value(
                    accessory, 'make', 'manufacturer'))
                _set_table_cell_text(doc, 3, row_idx, 2, _pick_item_value(
                    accessory, 'modelNo', 'model_no', 'model'))
                _set_table_cell_text(doc, 3, row_idx, 3, _pick_item_value(
                    accessory, 'serialNo', 'serial_no', 'serialNumber'))
                _set_table_cell_text(doc, 3, row_idx, 4, _pick_item_value(
                    accessory, 'functionPurpose', 'function_purpose', 'purpose', 'function'))
            cables = [
                ...
                if isinstance(item, dict) and any([
                    ...
                    _pick_item_value(item, 'powerSignal',
                                     'power_signal', 'type'),
                    _pick_item_value(item, 'shielded',
                                     'shielded_unshielded', 'shielding'),
                    _pick_item_value(item, 'functionPurpose',
                                     'function_purpose', 'purpose', 'function'),
                ])
            ]
            try:
                spacer_paragraph = _insert_paragraph_after_table(
                    doc.tables[3], '')
                spacer_format = spacer_paragraph.paragraph_format
                spacer_format.space_before = 0
                spacer_format.space_after = Inches(0.08)
            except Exception as exc:
                logger.debug(
                    'Unable to add spacer between accessories and cables tables: %s', exc)
            _ensure_table_repeated_rows(doc, 4, 2, len(cables))
            for idx, cable in enumerate(cables, start=2):
                row_idx = idx
                _set_table_cell_text(doc, 4, row_idx, 0, _pick_item_value(
                    cable, 'cableName', 'cable_name', 'name'))
                _set_table_cell_text(doc, 4, row_idx, 1, _pick_item_value(
                    cable, 'length', 'length_m'))
                _set_table_cell_text(doc, 4, row_idx, 2, _pick_item_value(
                    cable, 'powerSignal', 'power_signal', 'type'))
                _set_table_cell_text(doc, 4, row_idx, 3, _pick_item_value(
                    cable, 'shielded', 'shielded_unshielded', 'shielding'))
                _set_table_cell_text(doc, 4, row_idx, 4, _pick_item_value(
                    cable, 'functionPurpose', 'function_purpose', 'purpose', 'function'))
            eut = (_coerce_export_list(
                request_data.get('eut_specs')) or [{}])[0]
            _set_table_cell_text(doc, 5, 2, 0, _pick_item_value(
                eut, 'acVoltageRange', 'ac_voltage_range'))
            _set_table_cell_text(doc, 5, 2, 1, _pick_item_value(
                eut, 'acFreqRange', 'freqRange', 'freq_range'))
            _set_table_cell_text(doc, 5, 2, 2, _pick_item_value(
                eut, 'acDcInputCurrent', 'acInputCurrent', 'inputCurrent', 'input_current', 'ac_input_current'))
            _set_table_cell_text(doc, 5, 2, 3, _pick_item_value(
                eut, 'dcVoltageRange', 'dc_voltage_range'))
            _set_table_cell_text(doc, 5, 2, 4, _pick_item_value(
                eut, 'dcInputCurrent', 'dc_input_current'))
            _set_table_cell_text(doc, 5, 2, 5, _pick_item_value(
                eut, 'ratedPower', 'rated_power'))
            supply_rows = [
                item for item in _coerce_export_list(request_data.get('supply_vf'))
                if isinstance(item, dict) and any([
                    _pick_item_value(item, 'supplyVoltage',
                                     'voltage', 'supply_voltage'),
                    _pick_item_value(item, 'supplyFrequency',
                                     'frequency', 'supply_frequency'),
                    _pick_item_value(
                        item, 'remarks', 'notes', 'differentVoltageFrequency', 'different_voltage_frequency'),
                ])
            ]
            _ensure_table_repeated_rows(doc, 6, 2, len(supply_rows))
            for idx, supply in enumerate(supply_rows, start=2):
                _set_table_cell_text(doc, 6, idx, 0, _pick_item_value(
                    supply, 'supplyVoltage', 'voltage', 'supply_voltage'))
                _set_table_cell_text(doc, 6, idx, 1, _pick_item_value(
                    supply, 'supplyFrequency', 'frequency', 'supply_frequency'))
                _set_table_cell_text(doc, 6, idx, 2, _pick_item_value(
                    supply, 'remarks', 'notes', 'differentVoltageFrequency', 'different_voltage_frequency'))
            has_wireless = _norm_yes_no(
                request_data.get('has_wireless_interface'))
            _set_table_cell_text(
                doc, 7, 1, 1, f"{_box('Yes', has_wireless == 'yes')}    {_box('No', has_wireless == 'no')}")
            wireless_rows = _coerce_export_list(request_data.get('wireless'))
            _set_table_cell_text(doc, 7, 3, 0, _join([_pick_item_value(
                item, 'type', 'interfaceType', 'interface_type') for item in wireless_rows]))
            _set_table_cell_text(doc, 7, 3, 1, _join([_pick_item_value(
                item, 'carrierFrequency', 'carrier_frequency') for item in wireless_rows]))
            _set_table_cell_text(doc, 7, 3, 2, _join([_pick_item_value(
                item, 'maxOutputPower', 'max_output_power') for item in wireless_rows]))
            _set_table_cell_text(doc, 7, 3, 3, _join([_pick_item_value(
                item, 'clockFrequencies', 'clock_frequencies') for item in wireless_rows]))
            _set_table_cell_text(doc, 7, 3, 4, _join([_pick_item_value(
                item, 'functionPurpose', 'function_purpose', 'purpose', 'function') for item in wireless_rows]))
            _insert_block_diagram_in_template(
                doc, request_data.get('block_diagram'))
            _set_table_cell_text(
                doc, 9, 1, 0, request_data.get('product_description'))
            _set_table_cell_text(
                doc, 9, 3, 0, request_data.get('test_configuration'))
            standards = _selected_option_set(
                request_data.get('product_standards'))
            _set_table_cell_text(doc, 10, 0, 1, '    '.join([
                _box('IEC 61326-1:2020',
                     _selection_contains(standards, 'iec 61326-1')),
                _box('EN 61326-1:2021', _selection_contains(standards, 'en 61326-1')),
                _box('IEC 60601-1-2: 2014+A1:2020',
                     _selection_contains(standards, 'iec 60601-1-2')),
                _box('EN 60601-1-2: 2015+A1:2021',
                     _selection_contains(standards, 'en 60601-1-2')),
                _box('FCC Subpart 15B: 2024',
                     _selection_contains(standards, 'fcc')),
                _box('ICES-001 Issue 5: 2020',
                     _selection_contains(standards, 'ices')),
                _box('Other', _selection_contains(standards, 'other')),
            ]))
            env = _selected_option_set(request_data.get('product_environment'))
            _set_table_cell_text(doc, 10, 2, 1, '    '.join([
            ...
            _set_table_cell_text(doc, 10, 3, 1, '    '.join([
                _box('Home Healthcare', _selection_contains(
                    env, 'home healthcare', 'home')),
                _box('Professional Healthcare', _selection_contains(
                    env, 'professional healthcare', 'professional')),
            ]))
            grp = _selected_option_set(request_data.get('group'))
            _set_table_cell_text(doc, 10, 4, 1, _box(
                'Group 1', _selection_contains(grp, 'group 1')))
            _set_table_cell_text(doc, 10, 4, 2, _box(
                'Group 2', _selection_contains(grp, 'group 2')))
            cls = _selected_option_set(request_data.get('class_type'))
            _set_table_cell_text(doc, 10, 5, 1, _box(
                'Class A', _selection_contains(cls, 'class a')))
            _set_table_cell_text(doc, 10, 5, 2, _box(
                'Class B', _selection_contains(cls, 'class b')))
            selected_tests = set()
            selected_test_shortcuts = set()
            for source in [
                request_data.get('selected_tests'),
                ...
                if source not in (None, '', [], {}):
                    ...
                    selected_test_shortcuts.update(
                        _extract_selected_test_shortcuts(source))
            ...
            if test_hours:
                selected_tests.update(
                    _selected_option_set(list(test_hours.keys())))
                selected_test_shortcuts.update(
                    _extract_selected_test_shortcuts(list(test_hours.keys())))
            ...
            test_blocks = [
                (0, 'Conducted Emission (CE)', ('CE',), [
                    _join([
                        ...
                        _line('Voltage/Frequency',
                              request_data.get('ce_voltage_freq')),
                ...
                (4, 'Radiated Emission (RE)', ('RE',), [
                    _join([
                        ...
                        _line('Voltage/Frequency',
                              request_data.get('re_voltage_freq')),
                ...
                (8, 'Harmonic Current Emission', ('Harmonic', 'HARMONIC'), [_join([_line('Equipment Class', request_data.get('harmonic_class')), _line(
                    'Voltage/Frequency', request_data.get('harmonic_voltage_freq'))]), _map_value(test_hours, 'Harmonic', 'HARMONIC'), _map_value(test_remarks, 'Harmonic', 'HARMONIC')]),
                (12, 'Voltage Changes, Voltage Fluctuations and Flicker Emission', ('VoltageFlicker', 'Flicker'), [_join([_line('Specification', request_data.get(
                    'flicker_voltage_freq'))]), _map_value(test_hours, 'VoltageFlicker', 'Flicker'), _map_value(test_remarks, 'VoltageFlicker', 'Flicker')]),
                (16, 'Electrostatic Discharge Immunity', ('ESD',), [_join([_line('Voltage/Frequency', request_data.get('esd_voltage_freq')), _line('Contact', request_data.get('esd_contact')), _line(
                    'Air', request_data.get('esd_air')), _line('Indirect', request_data.get('esd_indirect'))]), _map_value(test_hours, 'ESD'), _map_value(test_remarks, 'ESD')]),
                (20, 'Radiated Susceptibility/Immunity', ('RS_RI', 'RS'), [_join([_line('Frequency Range', rs_freq_range_display)]), _join([_line('Field Strength 1', request_data.get('rs_field_strength1')), _line('Field Strength 2', request_data.get(
                    'rs_field_strength2')), _line('Field Strength 3', request_data.get('rs_field_strength3')), _line('Voltage/Frequency', request_data.get('rs_voltage_freq'))]), _map_value(test_hours, 'RS_RI', 'RS'), _map_value(test_remarks, 'RS_RI', 'RS')]),
                (25, 'Radiated Susceptibility/Immunity (Interim Method)', ('RS_RI_Interim', 'RS_Interim'), [_join([_line('Frequency Range', rs_interim_freq_range_display)]), _join([_line('Field Strength 1', request_data.get('rs_interim_field_strength1')), _line('Field Strength 2', request_data.get(
                    'rs_interim_field_strength2')), _line('Field Strength 3', request_data.get('rs_interim_field_strength3')), _line('Voltage/Frequency', request_data.get('rs_interim_voltage_freq'))]), _map_value(test_hours, 'RS_RI_Interim', 'RS_Interim'), _map_value(test_remarks, 'RS_RI_Interim', 'RS_Interim')]),
                (30, 'Electrical fast transient/burst Immunity', ('EFT',), [_join([_line('Voltage/Frequency', request_data.get('eft_voltage_freq')), _line('Test Level 1', request_data.get('eft_test_level1')), _line('Test Level 2', request_data.get('eft_test_level2'))]), _join([_line('Power lines', request_data.get(
                    'eft_cables_power')), _line('Signal lines', request_data.get('eft_cables_signal'))]), _join([_line('Signal Line 1', request_data.get('eft_signal_line1')), _line('Signal Line 2', request_data.get('eft_signal_line2'))]), _map_value(test_hours, 'EFT'), _map_value(test_remarks, 'EFT')]),
                (36, 'Surge Immunity', ('Surge', 'SURGE'), [_join([_line('Voltage/Frequency', request_data.get('surge_voltage_freq')), _line('Common Mode', request_data.get('surge_cm1')), _line('Differential Mode', request_data.get('surge_dm1'))]), _join([_line('Power lines', request_data.get('surge_cables_power')), _line(
                    'Signal lines', request_data.get('surge_cables_signal'))]), _join([_line('Signal Line 1', request_data.get('surge_signal_line1')), _line('Signal Line 2', request_data.get('surge_signal_line2'))]), _map_value(test_hours, 'Surge', 'SURGE'), _map_value(test_remarks, 'Surge', 'SURGE')]),
                (42, 'Conducted RF Disturbance Immunity', ('CRF',), [_join([_line('Voltage/Frequency', request_data.get('crf_voltage_freq')), _line('Test Level 1', request_data.get('crf_test_level1')), _line('Test Level 2', request_data.get('crf_test_level2'))]), _join([_line('Power lines', request_data.get('crf_cables_power')), _line(
                    'Signal lines', request_data.get('crf_cables_signal'))]), _join([_line('Signal Line 1', request_data.get('crf_signal_line1')), _line('Signal Line 2', request_data.get('crf_signal_line2'))]), _join([_line('Frequency Range', crf_freq_range_display)]), _map_value(test_hours, 'CRF'), _map_value(test_remarks, 'CRF')]),
                (49, 'Power Frequency Magnetic Field Immunity', ('PFMF', 'POWER'), [_join([_line('Voltage/Frequency', request_data.get('power_freq_voltage_freq')), _line(
                    'Field Strength', request_data.get('power_freq_test_level'))]), _map_value(test_hours, 'PFMF', 'POWER'), _map_value(test_remarks, 'PFMF', 'POWER')]),
                (53, 'Voltage Dips, Short Interruptions and Voltage Variations Immunity', ('VoltageDips', 'VOLTAGE'), [_join([_line('Voltage Dip 1', request_data.get('voltage_dips_voltage_dip1')), _line('Voltage Dip 2', request_data.get('voltage_dips_voltage_dip2')), _line('Voltage Dip 3', request_data.get('voltage_dips_voltage_dip3'))]), _join([_line('Voltage Variations', request_data.get('voltage_dips_voltage_freq')), _line(
                    'Time 1', request_data.get('voltage_dips_time1')), _line('Time 2', request_data.get('voltage_dips_time2'))]), _join([_line('Short Interruption', request_data.get('voltage_dips_interruption')), _line('Time 3', request_data.get('voltage_dips_time3')), _line('Time 4', request_data.get('voltage_dips_time4'))]), _map_value(test_hours, 'VoltageDips', 'VOLTAGE'), _map_value(test_remarks, 'VoltageDips', 'VOLTAGE')]),
            ]
            rows_to_remove_from_tests_table = []
            for start_row, label, aliases, values in test_blocks:
                selected = _test_selected(label, *aliases)
                _set_table_cell_text(doc, 11, start_row, 0,
                                     _box(label, selected), bold=True)
                for offset, value in enumerate(values):
                    _set_table_cell_text(
                        doc, 11, start_row + offset, 2, value if selected else '')
                if selected and values:
                    remark_value = _coerce_export_text(values[-1])
                    if not remark_value:
                        rows_to_remove_from_tests_table.append(start_row + len(values) - 1)
            if rows_to_remove_from_tests_table:
                _remove_table_rows(doc, 11, rows_to_remove_from_tests_table)
            for row_idx, field in enumerate(['continue_testing', 'test_report_required', 'uncertainty_required', 'test_witness'], start=0):
                ...
                _set_table_cell_text(doc, 12, row_idx, 1,
                                     _box('YES', choice == 'yes'))
                _set_table_cell_text(doc, 12, row_idx, 2,
                                     _box('YES', choice == 'yes'))
                _set_table_cell_text(doc, 12, row_idx, 3,
                                     _box('NO', choice == 'no'))
            conformity = _norm_yes_no(request_data.get(
                'conformity_statement') or request_data.get('conformity_required') or request_data.get('conformityStatement') or request_data.get('conformityRequired'))
            decision_rule_selected = _selected_option_set(
                request_data.get('decision_rule') or request_data.get('decisionRule') or request_data.get('conformityDecisionRule'))
            decision_rule_lines = [
                'The decision rule followed by TFS Product Testing Lab will be any one of the defined below,',
                _box('As prescribed by the standard (Measured value or observation will be provided with the results)',
                     _selection_contains(decision_rule_selected, 'standard_measured')),
                _box('As prescribed by the standard inclusive of measurement uncertainty.',
                     _selection_contains(decision_rule_selected, 'standard_uncertainty')),
                _box('As per customer specifications',
                     _selection_contains(decision_rule_selected, 'customer_specs')),
                _box('As per customer specification inclusive of measurement uncertainty.',
                     _selection_contains(decision_rule_selected, 'customer_specs_uncertainty')),
            ]
            _set_table_cell_text(doc, 12, 4, 1, _box(
                'YES', conformity == 'yes'))
            _set_table_cell_text(doc, 12, 4, 2, '\n'.join(
                decision_rule_lines) if conformity == 'yes' else doc.tables[12].rows[4].cells[2].text)
            _set_table_cell_text(doc, 12, 4, 3, _box('NO', conformity == 'no'))
            _set_table_cell_text(doc, 12, 5, 1, _box('NO', conformity == 'no'))
            _set_table_cell_text(
                doc, 12, 5, 2, 'We will report the measured results and the uncertainty.' if conformity == 'no' else '')
            modes = _coerce_export_list(request_data.get('functional_modes'))
            _set_table_cell_text(
                doc, 13, 0, 1, request_data.get('number_of_modes') or (len(modes) if modes else ''))
            _set_table_cell_labeled_lines(
                doc,
                13,
                1,
                0,
                [
                    (f"Mode {chr(65 + idx)}:", _coerce_export_text(mode))
            ...
            _set_table_cell_text(doc, 14, 0, 3, _join([request_data.get(
                'requester_department'), request_data.get('requester_division')], ' / '))
            _set_table_cell_text(
                doc, 14, 1, 1, request_data.get('requester_email'))
            _set_table_cell_text(
                doc, 14, 1, 3, request_data.get('requester_contact'))
            _set_table_cell_text(doc, 14, 2, 1, _fmt_date(
                request_data.get('requester_date')))
            _set_table_cell_text(doc, 14, 2, 3, _format_signature_export_value(
                request_data.get('requester_signature')))
            sample_condition = _coerce_export_text(
                request_data.get('sample_condition')).lower()
            _set_table_cell_text(doc, 15, 0, 1, request_data.get(
                'job_number') or request_data.get('job_id'))
            _set_table_cell_text(
                doc, 15, 1, 1, f"{_box('Good', sample_condition == 'good')}    {_box('Bad', sample_condition == 'bad')}")
            cap = _norm_yes_no(request_data.get('capability_available'))
            _set_table_cell_text(
                doc, 15, 2, 1, f"{_box('YES', cap == 'yes')}    {_box('NO', cap == 'no')}")
            _set_table_cell_text(doc, 15, 3, 1, _fmt_date(
                request_data.get('sample_received_date')))
            _set_table_cell_text(doc, 15, 5, 2, _format_test_duration_days_only(
                request_data.get('test_duration'),
                request_data.get('test_commencement_date'),
                request_data.get('test_completion_date')))
            _set_table_cell_text(doc, 15, 6, 2, _fmt_date(
                request_data.get('test_commencement_date')))
            _set_table_cell_text(doc, 15, 7, 2, _fmt_date(
                request_data.get('test_completion_date')))
            _set_table_cell_text(
                doc, 16, 0, 1, request_data.get('lab_manager_name'))
            _set_table_cell_text(doc, 16, 0, 3, _fmt_date(
                request_data.get('test_commencement_date') or request_data.get('lab_manager_date')))
            _set_table_cell_signature(
                doc, 16, 1, 1, request_data.get('lab_manager_signature'))
            output_stream = BytesIO()
            doc.save(output_stream)
            ...
            logger.exception(
                'Rev1 TRF export failed for request %s', request_data.get('id'))
    ...
    def download_test_request_form_docx(request_id):
        ...
        try:
            ...
            filename_seed = (
                request_data.get('tco_id')
                or getattr(test_request, 'tco_id', None)
                or request_data.get('job_number')
                or f"REQ-{test_request.id}"
            )
    ...
    def get_test_request_by_tco(tco_id):
        """Get a test request by TCO ID for form prefill.

        Allows any logged-in user to lookup TCO details by TCO# to prefill forms.
        Returns product and test details but excludes tco_id and job_number
        to ensure form creates new TCO/Job entries. Requester info uses current user.
        """
        try:
            ...
            # Get the test request by TCO ID (no permission check - public TCO lookup)
            ...
            # Remove tco_id and job_number so form creates new entries
            # These will be generated fresh when user submits the form
            request_data.pop('tco_id', None)
            request_data.pop('job_number', None)
            # Clear requester details so they come from logged-in user
            # The frontend form will populate these from current_user context
            request_data['requester_name'] = ''
            request_data['requester_email'] = ''
            request_data['requester_phone'] = ''
            request_data['company_name'] = ''
            logger.debug(
                'Fetched TCO for prefill: TCO=%s (request_id=%s, product=%s)',
                normalized_tco_id,
                test_request.id,
                test_request.product_name
            )
            return jsonify({
                'success': True,
                'data': request_data
    ...
    def assign_tests_to_engineers(request_id):
        ...
        try:
            ...
            if has_laboratory_details:
                ...
                sample_received_date_value = _parse_iso_date(
                    sample_received_date_raw)
                ...
                test_commencement_date_value = _parse_iso_date(
                    test_commencement_date_raw)
                ...
                test_completion_date_value = _parse_iso_date(
                    test_completion_date_raw)
            ...
            def _remember_existing_assignment(payload):
                ...
                try:
                    total_hours = round(float(total_hours_raw), 2) if total_hours_raw not in (
                        None, '', []) else None
                except (TypeError, ValueError):
                    total_hours = None
                normalized_payload = {
                ...
                _record_schedule_snapshot(
                    start_date_obj, end_date_obj, total_hours)
                ...
                if existing_planner_entries:
                    ...
                    for existing_assignment in raw_existing_assignments:
                        ...
                        fallback_existing_schedule_snapshots.append(
                            _build_planner_conflict_snapshot(
                                existing_assignment)
                        )
            ...
            if normalized_laboratory_details is not None:
                ...
                test_request.capability_available = normalized_laboratory_details[
                    'capability_available']
                test_request.sample_received_date = normalized_laboratory_details[
                    'sample_received_date']
                test_request.test_duration = normalized_laboratory_details['test_duration']
                test_request.test_commencement_date = normalized_laboratory_details[
                    'test_commencement_date']
                test_request.test_completion_date = normalized_laboratory_details[
                    'test_completion_date']
            ...
            if not merge_existing_assignments:
                ...
                # ✅ FIX: When merging assignments, cancel old entries for tests being rescheduled
        ...
        try:
            def safe_file_size(path: str) -> int:
                ...
                try:
                    ...
                    logger.warning(
                        'Unable to read file size for %s: %s', path, exc)
                    return 0
            ...
            rs_interim_allowed = any(
                token.startswith(
                    'iec 60601-1-2') or token.startswith('en 60601-1-2')
            ...
            extracted_data = {
                ...
                'conformity_required': form_data.get('conformityStatement', '') or form_data.get('conformityRequired', ''),
                'conformity_statement': form_data.get('conformityStatement', '') or form_data.get('conformityRequired', ''),
                'decision_rule': form_data.get('decisionRule', []) or form_data.get('conformityDecisionRule', []),
        ...
        try:
            ...
            def _build_due_badge(due_date_value, eou_status=None):
                if not due_date_value:
                    return None
                days_remaining = (due_date_value - get_ist_now().date()).days
                if days_remaining < 0:
                    overdue_days = abs(days_remaining)
                    return {
                        'text': f"Overdue by {overdue_days} day{'s' if overdue_days != 1 else ''}",
                        'class_name': 'bg-red-100 text-red-800 border border-red-200'
                    }
                if days_remaining == 0:
                    return {
                        'text': 'Due Today',
                        'class_name': 'bg-amber-100 text-amber-800 border border-amber-200'
                    }
                if days_remaining == 1:
                    return {
                        'text': 'Due Tomorrow',
                ...
                upcoming_window_days = 60 if str(eou_status or '').strip().upper() == 'EOU' else 30
                if days_remaining <= upcoming_window_days:
                    return {
                        'text': f'Due in {days_remaining} days',
                        'class_name': 'bg-yellow-100 text-yellow-800 border border-yellow-200'
                    }
                return None
            for equipment in pagination.items:
                calibration_due_badge = _build_due_badge(
                    equipment.calibration_due_date,
                    getattr(equipment, 'eou_status', None)
                )
                ic_due_badge = _build_due_badge(
                    equipment.ic_due_date,
                    getattr(equipment, 'eou_status', None)
                ) if str(equipment.ic_required or '').strip().lower() == 'yes' else None
                maintenance_required_value = str(equipment.maintenance_required or '').strip().lower()
                maintenance_due_badge = _build_due_badge(
                    equipment.maintenance_due_date,
                    getattr(equipment, 'eou_status', None)
                ) if maintenance_required_value and maintenance_required_value != 'no' else None
                setattr(equipment, 'calibration_due_badge', calibration_due_badge)
                setattr(equipment, 'ic_due_badge', ic_due_badge)
                setattr(equipment, 'maintenance_due_badge', maintenance_due_badge)
                setattr(equipment, 'has_upcoming_due', any(
    ...
    @flask_app.route('/api/equipment/export/excel')
    @login_required
    def api_equipment_export_excel():
        """Export filtered equipment data to Excel."""
        if current_user.role not in ['admin', 'lab_engineer']:
            return jsonify({
                'success': False,
                'error': 'Access denied. Admin or Lab Engineer privileges required.'
        ...
        try:
            ...
            if search_term and search_term.strip():
                ...
                query = query.filter(
                    ...
                    (Equipment.test_type.ilike(search_filter)) |
                    (Equipment.test_name.ilike(search_filter)) |
                    (Equipment.status.ilike(search_filter))
                )
            equipment_rows = query.order_by(Equipment.name.asc()).all()
            workbook = Workbook()
            worksheet = workbook.active
            worksheet.title = 'Equipment'
            ...
            headers = [
                ...
                'Manufacturer Calibration Params',
                'Calibration Agency Params',
                'Document Link',
                'Status',
            ]
            ...
            for equipment in equipment_rows:
                worksheet.append([
                    equipment.sl_no or '',
                    equipment.asset_id or '',
                    equipment.type or '',
                    equipment.name or '',
                    equipment.make or '',
                    equipment.model_no or '',
                    equipment.serial_no or '',
                    equipment.location or '',
                    equipment.test_type or '',
                    equipment.test_name or '',
                    getattr(equipment, 'eou_status', None) or '',
                    equipment.calibration_required or '',
                    equipment.calibration_frequency or '',
                    _fmt_date(equipment.calibration_date),
                    _fmt_date(equipment.calibration_due_date),
                    equipment.calibration_status_col or '',
                    equipment.ic_required or '',
                    _fmt_date(equipment.ic_date),
                    _fmt_date(equipment.ic_due_date),
                    equipment.maintenance_required or '',
                    _fmt_date(equipment.maintenance_date),
                    _fmt_date(equipment.maintenance_due_date),
                    equipment.manufacturer_calibration_params or '',
                    equipment.calibration_agency_params or '',
                    equipment.document_link or '',
                    equipment.status or '',
                ])
            column_widths = {
                'A': 10,
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f'equipment_list_{timestamp}.xlsx'
            )
        except Exception as e:
            logger.error('Error exporting equipment to Excel: %s', e, exc_info=True)
            return jsonify({
                'success': False,
                'error': 'Error exporting equipment data to Excel'
            }), 500
    ...
    def user_management():
        ...
        try:
            ...
            lab_engineer_users = User.query.filter_by(role='lab_engineer').count()
            regular_users = User.query.filter_by(role='user').count()
            active_users = User.query.filter_by(is_active=True).count()
            user_growth_30d = User.query.filter(
                User.created_at >= get_ist_now() - timedelta(days=30)
            ).count()
            statistics = {
                'total': total_users,
                'admins': admin_users,
                'lab_engineers': lab_engineer_users,
                'users': regular_users,
                'active': active_users,
                'total_growth': user_growth_30d
            }
    ...
    def _get_assigned_tests_context():
        ...
        logger.info(
            f"Found {len(assigned_requests)} assigned request candidates")
        test_plans = []
        for request in assigned_requests:
            try:
                ...
                planner_entry_filters = [
                    PlannerEntry.test_request_id == request.id]
                if request.tco_id:
                    planner_entry_filters.append(
                        PlannerEntry.tco_id == request.tco_id)
                planner_entries = PlannerEntry.query.filter(
                ...
                # ✅ FIX: Filter out cancelled entries and handle rescheduled tests
                ...
                if not active_entries_by_test and planner_entries:
                    most_recent = max(
                        planner_entries, key=lambda e: e.updated_at)
                    active_entries_by_test[(
                        most_recent.test_name or '').strip()] = most_recent
                filtered_planner_entries = list(
                    active_entries_by_test.values())
                parsed_assignments = []
                assigned_test_keys = set()
                ...
                def derive_review_status(parent_status, remaining_test_names, assignment_payloads):
                    normalized_parent_status = normalize_review_status(
                        parent_status)
                    ...
                    return min(
                        candidate_statuses,
                        key=lambda status_label: status_priority.get(
                            status_label, 999)
                    )
                ...
                if filtered_planner_entries:
                        ...
                        try:
                            ...
                            assignment_payload = {
                                ...
                                'peer_review_assigned_at': safe_datetime_format(
                                    getattr(
                                        entry, 'peer_review_assigned_at', None)
                                ),
                                'peer_reviewer_name': peer_reviewer_name,
                    ...
                    # When legacy rows are found, backfill planner entries so assigned-tests
                    # can treat them like real assignments for datasheets/cancel actions.
                    try:
                        ...
                        raw_assignments = []
                    legacy_planner_entries_by_key = {}
                    for existing_entry in planner_entries:
                        existing_key = _normalize_assignment_test_key(
                            getattr(existing_entry, 'test_name', None)
                        )
                        existing_status = str(
                            getattr(existing_entry, 'status', '') or ''
                        ).strip().lower()
                        if existing_key and existing_status != 'cancelled':
                            legacy_planner_entries_by_key[existing_key] = existing_entry
                    created_legacy_entries = []
                    for assignment in raw_assignments:
                        test_name = assignment.get('test_name', 'Test')
                        assignment_key = _normalize_assignment_test_key(
                            test_name)
                        if not assignment_key or assignment_key in legacy_planner_entries_by_key:
                            continue
                        engineer_id_raw = assignment.get('engineer_id')
                        try:
                            engineer_id = int(engineer_id_raw) if engineer_id_raw not in (
                                None, '', []) else None
                        except (TypeError, ValueError):
                            engineer_id = None
                        engineer_name = str(assignment.get(
                            'engineer_name') or '').strip() or 'Unassigned'
                        start_date_raw = assignment.get('start_date')
                        end_date_raw = assignment.get('end_date')
                        start_time_raw = assignment.get('start_time')
                        end_time_raw = assignment.get('end_time')
                        try:
                            start_date_obj = datetime.strptime(
                                str(start_date_raw), '%Y-%m-%d').date() if start_date_raw else None
                            end_date_obj = datetime.strptime(
                                str(end_date_raw), '%Y-%m-%d').date() if end_date_raw else None
                            start_time_obj = datetime.strptime(
                                str(start_time_raw), '%H:%M').time() if start_time_raw else None
                            end_time_obj = datetime.strptime(
                                str(end_time_raw), '%H:%M').time() if end_time_raw else None
                        except (TypeError, ValueError):
                            logger.warning(
                                'Skipping legacy planner backfill for request %s test %s because the schedule could not be parsed.',
                                request.id,
                                test_name
                            )
                            continue
                        ...
                        legacy_entry = PlannerEntry(
                            test_person_name=engineer_name,
                            engineer_user_id=engineer_id,
                        ...
                        created_legacy_entries.append(
                            (assignment_key, legacy_entry))
                        legacy_planner_entries_by_key[assignment_key] = legacy_entry
                    if created_legacy_entries:
                        try:
                            db.session.commit()
                            planner_entries.extend(
                                [entry for _, entry in created_legacy_entries])
                            logger.info(
                                'Backfilled %s legacy planner entries for assigned-tests request %s.',
                                len(created_legacy_entries),
                                request.id
                            )
                        except Exception as exc:
                            db.session.rollback()
                            logger.error(
                                'Failed to backfill legacy planner entries for request %s: %s',
                                request.id,
                                exc
                            )
                            for assignment_key, _ in created_legacy_entries:
                                legacy_planner_entries_by_key.pop(
                                    assignment_key, None)
                    for assignment in raw_assignments:
                        assignment_key = _normalize_assignment_test_key(
                            assignment.get('test_name', 'Test')
                        )
                        linked_entry = legacy_planner_entries_by_key.get(
                            assignment_key) if assignment_key else None
                        assignment_payload = {
                            'id': getattr(linked_entry, 'id', None),
                            'test_name': assignment.get('test_name', 'Test'),
                            'engineer_name': getattr(linked_entry, 'test_person_name', None) or assignment.get('engineer_name', 'Unassigned'),
                            'engineer_id': getattr(linked_entry, 'engineer_user_id', None) or assignment.get('engineer_id'),
                            'engineer_user_id': getattr(linked_entry, 'engineer_user_id', None),
                            'start_date': safe_date_format(getattr(linked_entry, 'start_date', None)) or assignment.get('start_date', ''),
                            'end_date': safe_date_format(getattr(linked_entry, 'end_date', None)) or assignment.get('end_date', ''),
                            'start_time': safe_time_format(getattr(linked_entry, 'start_time', None)) or assignment.get('start_time', ''),
                            'end_time': safe_time_format(getattr(linked_entry, 'end_time', None)) or assignment.get('end_time', ''),
                            'total_hours': getattr(linked_entry, 'total_hours', None) if linked_entry else assignment.get('total_hours'),
                            'start_date_display': _format_date_display(safe_date_format(getattr(linked_entry, 'start_date', None)) or assignment.get('start_date')),
                            'end_date_display': _format_date_display(safe_date_format(getattr(linked_entry, 'end_date', None)) or assignment.get('end_date')),
                            'start_time_display': _format_time_display(safe_time_format(getattr(linked_entry, 'start_time', None)) or assignment.get('start_time')),
                            'end_time_display': _format_time_display(safe_time_format(getattr(linked_entry, 'end_time', None)) or assignment.get('end_time')),
                            'status': getattr(linked_entry, 'status', None) or 'in_progress',
                            'datasheet_file_path': getattr(linked_entry, 'datasheet_file_path', None),
                            'datasheet_uploaded_at': safe_datetime_format(getattr(linked_entry, 'datasheet_uploaded_at', None)),
                            'datasheet_uploaded_by': getattr(linked_entry, 'datasheet_uploaded_by', None),
                            'peer_reviewer_user_id': getattr(linked_entry, 'peer_reviewer_user_id', None),
                            'peer_review_assigned_at': safe_datetime_format(getattr(linked_entry, 'peer_review_assigned_at', None)),
                            'peer_reviewer_name': None,
                            'datasheet_comments': getattr(linked_entry, 'datasheet_comments', None),
                        }
                        if is_meaningful_assignment(assignment_payload):
                            if assignment_key:
                                assigned_test_keys.add(assignment_key)
                ...
                remaining_tests = []
                for selected_test in selected_tests:
                    selected_key = _normalize_assignment_test_key(
                        selected_test)
                    if selected_key and selected_key in assigned_test_keys:
                        continue
                    remaining_tests.append(selected_test)
                ...
                # ✅ FIX: Get report fields from filtered_planner_entries directly
                ...
                logger.error(traceback.format_exc())
                continue
        total_tests_count = len(test_plans)
        draft_report_count = sum(
            1 for plan in test_plans
            if str(plan.get('status_display') or plan.get('status') or '').strip() == 'Draft Report'
        )
        datasheet_uploaded_count = sum(
            1 for plan in test_plans
            if str(plan.get('status_display') or plan.get('status') or '').strip() == 'Datasheet Uploaded'
        )
                ...
                <div class="content">
                    ...
                    <div class="alert-box">
                        <p style="margin: 0; color: #991b1b; font-weight: bold;">
                            ⚠��? The following event has been <strong>CANCELLED</strong> by {cancelled_by_user.username}
                        </p>
                    </div>
    ...
    ⚠��? The following event has been CANCELLED by {cancelled_by_user.username}:
    ...
    def delete_planner_entry(planner_id):
        ...
        try:
            ...
            current_app.logger.error(
                f'Error deleting planner entry {planner_id}: {e}')
            return jsonify({
                'success': False,
    ...
    def dashboard():
        ...
        try:
            ...
            completed_last_30 = sum(
                ...
                _count_in_window(
                    request_obj.updated_at or request_obj.created_at, last_30_days, now)
            )
            ...
            for index, (label, count) in enumerate(selected_test_counter.most_common(4)):
                percent = round((count / total_selected_tests)
                                * 100) if total_selected_tests else 0
            ...
            for request_obj in requests[:5]:
                status_text = str(
                    request_obj.status or 'Unknown').strip() or 'Unknown'
            ...
            for entry in planner_entries:
                ...
                if not engineer_name:
                    continue
                utilization_by_engineer[engineer_name] += float(
                    entry.total_hours or 1)
            max_utilization = max(utilization_by_engineer.values(), default=0)
            utilization_colors = ['bg-blue-500',
                                  'bg-green-500', 'bg-yellow-500']
            lab_utilization = []
            for index, (engineer_name, total_hours) in enumerate(utilization_by_engineer.most_common(3)):
                percent = round((total_hours / max_utilization)
                                * 100) if max_utilization else 0
            ...
            for delta_months in range(-5, 1):
                year_value, month_value = _shift_month(
                    today.year, today.month, delta_months)
                month_start = datetime(year_value, month_value, 1, tzinfo=IST)
                next_year, next_month = _shift_month(
                    year_value, month_value, 1)
                next_month_start = datetime(
                    next_year, next_month, 1, tzinfo=IST)
                request_count = sum(
                    1 for request_obj in requests
                    if _count_in_window(
                        request_obj.submitted_at or request_obj.created_at,
            ...
            for item in monthly_trends:
                item['height_pct'] = round(
                    (item['count'] / max_monthly_count) * 100) if max_monthly_count else 0
            ...
            # Calculate testing hours completed (YTD)
            ytd_start = datetime(now.year, 1, 1, tzinfo=IST)

            def _as_date(value):
                """planner_entries.completion_date is DATETIME in the database while the
                model declares Date, so this attribute comes back as a datetime for some
                rows and a date for others. Comparing the two raises TypeError, which took
                the whole dashboard down, so both sides are reduced to a date here."""
                return value.date() if isinstance(value, datetime) else value

            total_testing_hours = sum(
                float(entry.total_hours) or 0
                for entry in planner_entries
                if entry.total_hours and entry.completion_date
                and _normalize_status(entry.status) != 'cancelled'
                and _as_date(entry.completion_date) >= ytd_start.date()
            )
            # Calculate estimated cost avoidance (estimate: $250/hour)
            cost_per_hour = 250
            estimated_cost_avoidance = int(total_testing_hours * cost_per_hour)
            # Calculate compliance projects completed (YTD)
            compliance_projects_completed = sum(
                1 for request_obj in requests
                if _normalize_status(request_obj.status) in completed_request_statuses
                and _count_in_window(request_obj.updated_at or request_obj.created_at, ytd_start, now)
                and any('compliance' in str(getattr(test, 'test_code', '')).lower()
                        for test in getattr(request_obj, 'tests', []))
            )
            # Test type vs hours and TCO count
            test_type_hours = {}
            test_type_tco = {}
            for entry in planner_entries:
                if entry.total_hours and _normalize_status(entry.status) != 'cancelled':
                    test_type = entry.test_name or 'Unknown'
                    test_type_hours[test_type] = test_type_hours.get(
                        test_type, 0) + (entry.total_hours or 0)
                    test_type_tco[test_type] = test_type_tco.get(
                        test_type, 0) + 1
            # Get top 3 test types
            top_test_types = sorted(
                test_type_tco.items(), key=lambda x: x[1], reverse=True)[:3]
            test_type_chart_data = []
            for test_type, count in top_test_types:
                hours = test_type_hours.get(test_type, 0)
                test_type_chart_data.append({
                    'label': test_type,
                    'count': count,
                    'hours': round(hours, 1)
                })
            # Test utilization by test name (with percentage and hours)
            test_name_hours = {}
            test_name_count = {}
            for entry in planner_entries:
                if entry.total_hours and _normalize_status(entry.status) != 'cancelled':
                    test_name = entry.test_name or 'Unknown'
                    test_name_hours[test_name] = test_name_hours.get(
                        test_name, 0) + (entry.total_hours or 0)
                    test_name_count[test_name] = test_name_count.get(
                        test_name, 0) + 1
            total_test_hours = sum(test_name_hours.values()) or 1
            test_utilization = []
            for test_name, hours in sorted(test_name_hours.items(), key=lambda x: x[1], reverse=True)[:5]:
                percentage = round((hours / total_test_hours) * 100, 1)
                test_utilization.append({
                    'name': test_name,
                    'percentage': percentage,
                    'hours': round(hours, 1),
                    'count': test_name_count.get(test_name, 0)
                })
            # Engineer performance: hours and requests per engineer
            engineer_hours = {}
            engineer_requests = {}
            for entry in planner_entries:
                engineer_name = entry.test_person_name or 'Unknown'
                if engineer_name and _normalize_status(entry.status) != 'cancelled':
                    engineer_hours[engineer_name] = engineer_hours.get(
                        engineer_name, 0) + (entry.total_hours or 0)
                    engineer_requests[engineer_name] = engineer_requests.get(
                        engineer_name, 0) + 1
            engineer_performance = []
            for engineer, hours in sorted(engineer_hours.items(), key=lambda x: x[1], reverse=True)[:5]:
                engineer_performance.append({
                    'name': engineer,
                    'hours': round(hours, 1),
                    'requests': engineer_requests.get(engineer, 0)
                })
            # Month wise service breakdown
            month_wise_service = {}
            for entry in planner_entries:
                if _normalize_status(entry.status) != 'cancelled':
                    month_key = (entry.start_date.year, entry.start_date.month) if entry.start_date else (
                        now.year, now.month)
                    month_wise_service[month_key] = month_wise_service.get(
                        month_key, 0) + 1
            # Hours consumption by TCO/test type
            hours_consumption_tco = []
            for test_type, hours in sorted(test_type_hours.items(), key=lambda x: x[1], reverse=True):
                hours_consumption_tco.append({
                    'label': test_type,
                    'hours': round(hours, 1),
                    'tco_count': test_type_tco.get(test_type, 0)
                })
            # Calculate avg hours per test
            avg_hours_per_test = round(
                total_test_hours / max(sum(test_type_tco.values()), 1), 2)
            # Utilization by Division pie chart data
            division_utilization = {}
            for request_obj in requests:
                if _normalize_status(request_obj.status) != 'cancelled':
                    division = request_obj.requester_division or 'Unknown'
                    division_utilization[division] = division_utilization.get(
                        division, 0) + 1
            division_chart_data = [
                {'label': division, 'value': count}
                for division, count in sorted(division_utilization.items(), key=lambda x: x[1], reverse=True)
            ]
            # Utilization by Division-Department pie chart data
            dept_utilization = {}
            for request_obj in requests:
                if _normalize_status(request_obj.status) != 'cancelled':
                    division = request_obj.requester_division or 'Unknown'
                    department = request_obj.requester_department or 'Unknown'
                    dept_key = f"{division}_{department}" if department else division
                    dept_utilization[dept_key] = dept_utilization.get(
                        dept_key, 0) + 1
            dept_chart_data = [
                {'label': dept_key, 'value': count}
                for dept_key, count in sorted(dept_utilization.items(), key=lambda x: x[1], reverse=True)
            ]
            # YTD Monthly volume trend for line chart
            ytd_monthly_volume = []
            monthly_totals = {}
            for month in range(1, now.month + 1):
                month_start = datetime(now.year, month, 1, tzinfo=IST)
                if month == now.month:
                    month_end = now
                else:
                    next_month = month + 1
                    month_end = datetime(now.year, next_month, 1, tzinfo=IST)
                ...
                # Count requests by service type for this month
                compliance_count = 0
                precompliance_count = 0
                da_count = 0
                for request_obj in requests:
                    if _count_in_window(request_obj.submitted_at or request_obj.created_at, month_start, month_end):
                        if hasattr(request_obj, 'service_types') and request_obj.service_types:
                            for service in request_obj.service_types:
                                if 'compliance' in service.service_type.lower() and 'pre' not in service.service_type.lower():
                                    compliance_count += 1
                                elif 'pre' in service.service_type.lower():
                                    precompliance_count += 1
                                elif 'da' in service.service_type.lower():
                                    da_count += 1
                        else:
                            # Default to DA if no service type specified
                            da_count += 1
                month_label = datetime(now.year, month, 1).strftime('%b')
                ytd_monthly_volume.append({
                    'label': month_label,
                    'compliance': compliance_count,
                    'precompliance': precompliance_count,
                    'da': da_count,
                    'total': month_requests,
                    'value': month_requests
                })
            ...
            # Add Grand Total row
            grand_total_compliance = sum(m['compliance']
                                         for m in ytd_monthly_volume)
            grand_total_precompliance = sum(
                m['precompliance'] for m in ytd_monthly_volume)
            grand_total_da = sum(m['da'] for m in ytd_monthly_volume)
            grand_total_all = sum(m['total'] for m in ytd_monthly_volume)
            ytd_monthly_volume.append({
                'label': 'Grand Total',
                'compliance': grand_total_compliance,
                'precompliance': grand_total_precompliance,
                'da': grand_total_da,
                'total': grand_total_all,
                'value': grand_total_all,
                'is_grand_total': True
            })
            # Update active tests to highlight active test requests
            active_test_requests = sum(
                1 for request_obj in requests
                if _normalize_status(request_obj.status) in active_request_statuses
            )
            dashboard_data = {
                'summary': {
                    ...
                    'active_test_requests': active_test_requests,
                    ...
                    'cost_avoidance_text': f'${estimated_cost_avoidance/1000:.0f}K',
                    'compliance_projects_completed': compliance_projects_completed,
                    'avg_hours_per_test': avg_hours_per_test,
                },
                ...
                'test_type_chart_data': test_type_chart_data,
                'test_utilization': test_utilization,
                'engineer_performance': engineer_performance,
                'month_wise_service': month_wise_service,
                'hours_consumption_tco': hours_consumption_tco,
    ...
    def upload_report(request_id):
        ...
        for entry in planner_entries:
            ...
            if entry.status != 'cancelled':
                entry.status = 'Admin Sign Off' if skips_report_review else 'report_uploaded'
            entry.updated_at = now
        # Step 6: Update EMC request row.
        test_request.status = 'Admin Sign Off' if skips_report_review else 'Draft Report'
        test_request.updated_at = now
        ...
        new_status = 'Admin Sign Off' if skips_report_review else 'Draft Report'
        return jsonify({
            'success': True,
            'message': (
                'Report uploaded successfully. Awaiting Admin Sign Off.'
    ...
    def admin_sign_off(request_id):
        ...
        try:
            ...
            if test_request.status != 'Proceed Report':
                # Build dual filter: test_request_id OR tco_id
                planner_filters = [
                    PlannerEntry.test_request_id == test_request.id]
                if test_request.tco_id:
                    planner_filters.append(
                ...
                # Check if planner entries indicate approval status
                planner_can_proceed = any(
                    e.status in ('Proceed Report',
                ...
                current_app.logger.warning(
                    "Admin sign-off for request %d: parent status '%s' didn't match 'Proceed Report', "
    ...
    def resync_request_status(request_id):
        """Force-recompute parent request status from planner entries (admin-only).

        Used to fix stuck records where parent status is out of sync with actual
        planner entry states. Queries planner entries using dual filter (test_request_id OR tco_id).
        """
        if current_user.role != 'admin':
        ...
        try:
            ...
            # Recompute status from planner entries (same logic as _update_parent_request_datasheet_status)
            old_status = test_request.status
            active_entries = [
                e for e in all_entries if e.status != 'cancelled']
            ...
            current_app.logger.info(
                "Resync status for request %d by admin %s: %s -> %s",
            ...
            db.session.rollback()
            current_app.logger.error(
                f'Error resyncing request {request_id} status: {e}')
            return jsonify({'success': False, 'error': str(e)}), 500
    ...
    def submit_report_access_feedback(request_id):
        ...
        data = request.get_json() or {}
        ratings, feedback_comments, validation_error = _parse_report_access_feedback_payload(
            data)
        ...
        try:
            feedback_comment = _format_report_access_feedback_comment(
                ratings, feedback_comments
            )
            _append_review_comment_entry(
                test_request=test_request,
            ...
            db.session.rollback()
            logger.error(
                f'Error submitting report access feedback for request {request_id}: {e}')
            return jsonify({
                'success': False,
    ...
    def proceed_report(request_id):
        ...
        try:
            ...
            # FALLBACK: If parent status is not approvable, check planner entries
            # using dual filter. If planner entries show approvable status,
            # allow the transition anyway.
            if not _report_is_approvable_status(current_status):
                ...
                planner_entries = PlannerEntry.query.filter(
                    db.or_(*planner_filters)).all()
                # Check if planner entries indicate a draft report is ready
                # (i.e., they have a report file and are in a state that suggests draft report exists)
                planner_has_draft_report = any(
                    e.report_file_path and _report_is_approvable_status(
                        (e.status or '').strip().lower()
                    )
                    for e in planner_entries
                    if e.status != 'cancelled'
                )
                ...
                current_app.logger.warning(
                    "Proceed report for request %d: parent status '%s' wasn't approvable, "
    ...
    def _serialize_peer_reviewer_candidates(exclude_user_id=None):
        reviewers = _get_peer_reviewer_candidates(
        ...
        for paragraph in doc.paragraphs:
            ...
            if normalized_caption in paragraph_text:
                try:
                    ...
                    logger.warning(
                        'Unable to insert Surge image for %s: %s', caption_text, exc)
                return
            ...
            if normalized_target in paragraph_text:
                ...
                for image_value in image_values:
                    ...
                    if not image_bytes:
                        continue
                    try:
                        image_paragraph = _insert_paragraph_after(
                            anchor_paragraph, '')
                        image_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        image_paragraph.add_run().add_picture(
                            BytesIO(image_bytes),
                        ...
                        logger.warning(
                            'Unable to insert Surge functional check image: %s', exc)
                return
    def _update_parent_request_datasheet_status(assignment):
        ...
        Queries planner entries by BOTH test_request_id and tco_id (dual filter),
        ensuring all entries linked to a parent request are considered.
        """
        db.session.flush()
        # Build dual filter: test_request_id OR tco_id
        planner_filters = [PlannerEntry.test_request_id ==
                           assignment.test_request_id]
        if assignment.tco_id:
            planner_filters.append(PlannerEntry.tco_id == assignment.tco_id)
        all_entries = PlannerEntry.query.filter(db.or_(*planner_filters)).all()
        if not all_entries:
            return
        # ✅ FIX: Only consider active (non-cancelled) entries for terminal state check
    ...
    def _format_checkbox_option_line(value, options):
        ...
        matched_any = any(option_key in tokens for _,
                          option_key in normalized_options)
        if not matched_any:
    ...
    def _build_surge_datasheet_docx(template_path, output_path, parent_request, assignment, form_data):
        ...
        specification_values = [
            ...
            _format_checkbox_option_line(form_value('test_voltage_cm'), [
                                         '�0.5', '�1', '�2', '�4']),
            _format_checkbox_option_line(form_value('test_voltage_dm'), [
                                         '�0.5', '�1', '�2', '�4']),
            form_value('coupling_phases'),
            form_value('repetition_rate'),
            form_value('test_mode'),
            form_value('eut_configuration'),
            form_value('eut_input_voltage_freq',
                       request_data.get('surge_voltage_freq') or ''),
            form_value('ambient_temperature'),
            form_value('relative_humidity'),
            form_value('test_date'),
            form_value(
                'tested_by', assignment.test_person_name or parent_request.assigned_engineer_name or '')
        ]
        for row_idx, row_value in enumerate(specification_values):
            if row_idx < len(test_spec_table.rows):
                for col_idx in range(2, len(test_spec_table.rows[row_idx].cells)):
                    test_spec_table.rows[row_idx].cells[col_idx].text = _datasheet_text(
                        row_value)
        equipment_rows = []
        ...
        for index in range(equipment_count):
            equipment_rows.append([
                ...
                equipment_models[index] if index < len(
                    equipment_models) else '',
                equipment_serials[index] if index < len(
                    equipment_serials) else '',
                equipment_cal_dues[index] if index < len(
                    equipment_cal_dues) else '',
            ])
        if equipment_rows:
            _safe_repeating_row_values(doc.tables[1], 1, equipment_rows)
        modification_rows = []
        modification_states = form_values('surge_modification_state[]')
        modification_descriptions = form_values(
            'surge_modification_description[]')
        ...
        for index in range(modification_count):
            modification_rows.append([
                modification_states[index] if index < len(
                    modification_states) else '',
                modification_descriptions[index] if index < len(
                    modification_descriptions) else '',
                modification_fitted_by[index] if index < len(
                    modification_fitted_by) else '',
                modification_dates[index] if index < len(
                    modification_dates) else '',
            ])
        ...
        def fill_power_table(table, segment_start):
            ...
            positive_values = {field: form_values(
                field) for field in pos_field_names}
            negative_values = {field: form_values(
                field) for field in neg_field_names}
                ...
                if row_idx < len(table.rows):
                    for col_offset, field in enumerate(pos_field_names, start=1):
                        ...
                        table.rows[row_idx].cells[col_offset].text = (
                            _datasheet_text(values[value_index]) if value_index < len(
                                values) else ''
                        )
        ...
        for index in range(software_count):
            software_rows.append([
                ...
                software_versions[index] if index < len(
                    software_versions) else ''
            ])
        if software_rows:
            _safe_repeating_row_values(doc.tables[6], 1, software_rows)
        _safe_table_cell_text(doc.tables[7], 0, 1, form_value(
            'required_performance_criteria'))
        _safe_table_cell_text(
            doc.tables[7], 1, 1, form_value('met_performance_criteria'))
        ...
        if not isinstance(setup_images, list):
            ...
            legacy_additional_images = form_data.get(
                'surge_additional_setup_images')
            if isinstance(legacy_additional_images, list):
                setup_images.extend(
                    [img for img in legacy_additional_images if img])
    ...
    def generate_surge_datasheet():
        ...
        try:
            ...
            test_request_id = payload.get(
                'test_request_id') or payload.get('test_plan_id')
            ...
            if (
                ...
                effective_peer_reviewer_id = str(
                    assignment.peer_reviewer_user_id)
            ...
            if test_date_raw:
                try:
                    generated_test_date = datetime.strptime(
                        test_date_raw, '%Y-%m-%d').date()
                    ...
                    logger.warning(
                        'Unable to parse generated Surge test date: %s', test_date_raw)
    ...
    def cancel_test_assignment():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(
                f"Error cancelling test assignment: {str(e)}")
    ...
    def admin_approval():
        ...
        try:
            ...
            for request in at_review_requests:
                ...
                test_plans.append({
                    ...
                    'reviewed_at': latest_shared_comment.get('created_at') if latest_shared_comment else (
                        request.reviewed_at.isoformat() if getattr(
                            request, 'reviewed_at', None) else None
                    )
                })
            ...
            statistics = _build_request_status_summary(EMCRequest.query)
    ...
    def admin_completed(request_id):
        ...
        try:
            ...
            test_request.reviewed_by = current_user.username
            test_request.reviewed_at = get_ist_now()
            # Update all planner entries for this request using dual filter
            planner_filters = [PlannerEntry.test_request_id == test_request.id]
            if test_request.tco_id:
                planner_filters.append(
                    PlannerEntry.tco_id == test_request.tco_id)
            ...
            db.session.commit()
            # Send completion notification to requester and admin
            send_completion_notification(
                test_request=test_request,
                completed_by=current_user.username
            )
            return jsonify({
                'success': True,
                'message': 'Report signed off successfully. Status updated to Completed. Notification email sent to requester and admin.'
            })
        except Exception as e:
            db.session.rollback()
        ...
        try:
            ...
            for test_request in requests:
                ...
                test_plans.append({
                    ...
                    'reviewed_at': latest_shared_comment.get('created_at') if latest_shared_comment else (
                        test_request.reviewed_at.isoformat() if hasattr(
                            test_request, 'reviewed_at') and test_request.reviewed_at else None
                    )
                })
            if sort_by:
                test_plans = _apply_request_identifier_sort(
