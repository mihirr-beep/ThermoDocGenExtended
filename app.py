...
from flask import Flask, current_app, render_template, request, jsonify, send_file, flash, redirect, url_for, send_from_directory, abort
...
"""
        ...
        try:
            ...
            try:
                if current_user.role == 'admin':
                    ...
                    base_query = EMCRequest.query.filter(
                        db.or_(
                            EMCRequest.user_id == current_user.id,
                            non_draft_filter
                        )
                    )
                    logger.info(
            ...
            #  Ordering & pagination                                               #
            # ------------------------------------------------------------------ #
            query = query.order_by(EMCRequest.created_at.desc())

            pagination = query.paginate(
                page=page, per_page=per_page, error_out=False)
            test_requests = pagination.items

            logger.info(
                f'Found {len(test_requests)} test requests '
                f'(page {pagination.page}/{pagination.pages}, total: {pagination.total})'
            )
    ...
    def approve_peer_review(planner_id):
        """Approve a planner entry's datasheet for peer review"""
        ...
        try:
            ...
            # Verify it's in Peer Review status
            if entry.status != 'Peer Review':
                return jsonify({
                    'success': False,
                    'error': f'Entry is not in Peer Review status. Current status: {entry.status}'
                }), 400
            ...
            # Check if all planner entries for this test request are completed
            # If so, update the parent test request status too
            if entry.test_request_id:
                all_entries = PlannerEntry.query.filter_by(
                    test_request_id=entry.test_request_id
                ).filter(
                    PlannerEntry.status != 'cancelled'  # Ignore cancelled entries
                ).all()

                all_completed = all(
                    e.status == 'completed' for e in all_entries)

                if all_completed:
                    test_request = _resolve_request(entry.test_request_id)
                    if test_request:
                        test_request.status = 'Completed'
                        test_request.updated_at = get_ist_now()
                        logger.info(
                            f'All entries completed for test request {entry.test_request_id}, '
            ...
            return jsonify({
                'success': True,
                'message': 'Peer review approved successfully',
                'data': {
                    'id': entry.id,
                    'status': entry.status,
    ...
    def get_peer_review_entries():
        """Get planner entries that are completed and ready for peer review"""
        ...
        try:
            ...
            result = []
            for entry, actual_tco_id, product_name, requester_name, actual_request_id in results:
                # Log for debugging
    ...
    def _extract_selected_test_shortcuts(value) -> set:
        ...
        def _collect_tokens(raw_value):
            ...
            if isinstance(raw_value, dict):
                ...
                # Handle object-style entries such as:
                # {"shortcut": "CE"}, {"value": "RE"}, {"test_name": "Harmonic"}
                ...
                # Handle map-style selections such as:
                # {"CE": true, "RE": false}
    ...
    def get_lab_engineers():
        ...
        try:
            ...
            lab_engineers = User.query.filter(
    ...
    def assign_tests_to_engineers(request_id):
        ...
        try:
            ...
            if not merge_existing_assignments and not isinstance(laboratory_details_raw, dict):
                return jsonify({
                    'success': False,
                    'error': 'Laboratory details are required'
                }), 400
            ...
            if isinstance(laboratory_details_raw, dict):
            ...
            elif merge_existing_assignments:
    ...
    def create_test_plan():
        ...
        try:
            ...
            if is_update and tco_id_from_request:
                ...
                if _is_draft_status(existing_request.status) and not _is_request_owner(existing_request):
                    logger.warning(
                ...
                if existing_request.status not in ['Draft', 'Need More Information']:
                    logger.error(
    ...
    def _get_assigned_tests_context():
        ...
        assigned_statuses = ['Assigned Lab Engineer', 'Update plan',
                             'Need More Information', 'Test Plan Approved', 'Draft Report', 'Datasheet Uploaded', 'Proceed Report', 'Admin Sign Off', 'Completed', 'Assigned', 'Test Schedule In Progress', 'In Progress', 'Report Uploaded', 'report_uploaded', 'Peer Review']
        approved_statuses = ['Approved', 'Test Plan Approved', 'Completed']

        assigned_requests = EMCRequest.query.filter(
        ...
        for request in assigned_requests:
            try:
                selected_tests = []
                for source in (
                    getattr(request, 'selected_tests', None),
                    getattr(request, 'selected_tests_for_development', None),
                ):
                    if not source:
                        continue
                ...
                # âœ… FIX: Get report fields from planner_entries directly
        ...
        test_plans.sort(key=lambda x: 1 if x['status'] == 'Completed' else 0)
    ...
    def review():
        ...
        try:
            ...
            return render_template('review.html', test_plans=test_plans, statistics=statistics)
        except Exception as e:
    ...
    def upload_report(request_id):
        ...
        for entry in planner_entries:
            ...
            if entry.status != 'cancelled':
                entry.status = 'report_uploaded'

        # Step 6: Update EMC request row.
        test_request.status = 'Draft Report'
    ...
    def assigned_test():
        ...
        try:
            test_plans, statistics = _get_assigned_tests_context()
            return render_template('assigned_test.html',
    ...
    def _update_parent_request_datasheet_status(assignment):
        ...
        if (
            ...
            current_app.logger.info(
                "All entries terminal for request %s. Parent status updated to 'Datasheet Uploaded'.",
    ...
    def generate_surge_datasheet():
        ...
        try:
            ...
            return jsonify({
                'success': True,
                'message': 'Surge datasheet generated successfully',
                'filename': filename,
    ...
    def upload_test_datasheet():
        try:
            ...
            if not all([start_date_str, end_date_str, start_time_str, end_time_str]):
                return jsonify({'success': False, 'message': 'Start date/time and end date/time are required'}), 400
            ...
            if start_date_str and start_time_str:
                try:
                    ...
                    return jsonify({'success': False, 'message': 'Invalid start date/time format. Expected YYYY-MM-DD HH:MM'}), 400
            ...
            if end_date_str and end_time_str:
                try:
                    ...
                    return jsonify({'success': False, 'message': 'Invalid end date/time format. Expected YYYY-MM-DD HH:MM'}), 400

            # Validate end is after start
            if start_datetime and end_datetime and end_datetime <= start_datetime:
                return jsonify({'success': False, 'message': 'End date/time must be after start date/time'}), 400
            ...
            # Update with actual test execution dates/times
            if start_datetime:
                assignment.start_date = start_datetime.date()
                assignment.start_time = start_datetime.time()

            if end_datetime:
                assignment.end_date = end_datetime.date()
                assignment.end_time = end_datetime.time()
                assignment.completion_date = end_datetime.date()
            ...
            current_app.logger.info(
                ...
                f"Test period: {start_datetime} to {end_datetime} "
                f"({total_hours} hours)"
            )
            ...
            return jsonify({
                ...
                'filename': filename,
                'test_duration_hours': total_hours
            })
    ...
    def admin_approval():
        ...
        try:
            ...
            at_review_requests = EMCRequest.query.filter(
