-- ==========================================================
-- Datasheet normalisation - proposed DDL
-- GENERATED from the 11 live schemas: every column name below is a
-- real form key. MySQL 8, InnoDB, utf8mb4.
-- ==========================================================

-- ---------- 1. HEADER: one row per test execution ----------
...
-- ---------- 2. ELEVEN per-test tables ----------

CREATE TABLE `datasheet_ce` (          -- Conducted Emission
...
CREATE TABLE `datasheet_voltagedips` (          -- Voltage Dips & Interruptions
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_voltageflicker` (          -- Flicker
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_harmonic` (          -- Harmonics
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_eft` (          -- Electrical Fast Transient
  `datasheet_id` INT PRIMARY KEY,
  -- 8 field(s) unique to this test
...
CREATE TABLE `datasheet_esd` (          -- Electrostatic Discharge
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_surge` (          -- Surge
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_re` (          -- Radiated Emission
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_rs_ri` (          -- Radiated Susceptibility
  `datasheet_id` INT PRIMARY KEY,
...
CREATE TABLE `datasheet_crf` (          -- Conducted RF Immunity
  `datasheet_id` INT PRIMARY KEY,
  -- 6 field(s) unique to this test
...
CREATE TABLE `datasheet_pfmf` (          -- Power-Frequency Magnetic Field
  `datasheet_id` INT PRIMARY KEY,
  -- 4 field(s) unique to this test
  `test_level`                                         VARCHAR(40) NULL,
...
-- ---------- 3. shared child tables (all 11 tests) ----------

CREATE TABLE `datasheet_equipment` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE `datasheet_software` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `datasheet_id` INT NOT NULL, `row_no` INT NOT NULL,
...
CREATE TABLE `datasheet_modification` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `datasheet_id` INT NOT NULL, `row_no` INT NOT NULL,
...
-- ---------- 4. OBSERVATIONS + their legend ----------
-- The cell verdicts. Codes are A/B/C/D plus the extended set the form offers:
--   B1 B2 B3  C1 C2 C3  D1 D2 D3  (and NA)
...
-- What each code MEANS on this datasheet. The engineer types one description
-- per code they used, e.g. B2 = "display flickered, recovered without action".
-- Posted as <prefix>_code[] / <prefix>_desc[]; today it only exists in form_json.
...
-- ---------- summary ----------
-- table                        spec imgslots  grids   cols
-- datasheet_ce                   24      6      2     27
-- datasheet_voltagedips           3      2      3      7
-- datasheet_voltageflicker        5      2      3      9
-- datasheet_harmonic              8      2      2     11
-- datasheet_eft                   8      6      4     13
-- datasheet_esd                   6      5      3     10
-- datasheet_surge                 4      6      3      8
-- datasheet_re                   51     11      2     54
-- datasheet_rs_ri                17      5      1     19
-- datasheet_crf                   6      3      1      8
-- datasheet_pfmf                  4      5      1      6
-- 11 per-test tables + 1 header + 3 shared + 2 observation + 2 audit = 19 tables
