...
def ingest_datasheet(docx_path, planner_entry_id, code, meta=None, logger=None):
    ...
    try:
        ...
        from . import tracing
        tracing.setup_tracing()   # so the embedding calls below are observed in Langfuse
        ...
        # embed at the LIVE index's dimension so vectors always fit the index
        vecs = embeddings.embed_texts([c["text"] for c in chunks],
                                      dim=vector_store.index_dimension())
        base = dict(meta or {})
